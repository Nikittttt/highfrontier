...
class entry():
    ...
    def __init__(self, surface, topleft, width, max_letters, starting_text="",
                 restrict_input_to=" QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm1234567890"):
    ...
    def draw(self):
        ...
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0], self.topleft[1] + 1),
                         (self.topleft[0] + self.width - 1, self.topleft[1] + 1), 2)  # black horizontal
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0] + 1, self.topleft[1]),
                         (self.topleft[0] + 1, self.topleft[1] + self.height - 1), 2)  # black vertical
...
class vscrollbar():
    def __init__(self, surface, function, topleft, length_of_bar_in_pixel, range_of_values, range_seen=None,
                 start_position=0, function_parameter=None):
        """
        Draws a scroll bar
    ...
    def calculate_extent_of_slider(self):
        ...
        if not self.unmovable:
            if self.range_seen is None:
                ...
                percentage_position = float(self.position - self.range_of_values[0]) / float(
                    self.range_of_values[1] - self.range_of_values[0])
                ...
                percentage_taken_by_slider = float(self.range_seen) / float(
                    self.range_of_values[1] - self.range_of_values[0])
    ...
    def draw(self):
        ...
        pygame.draw.line(self.surface, (255, 255, 255), (self.topleft[0] + 2, self.topleft[1] + extent_of_slider[0]),
                         (self.topleft[0] + 2, self.topleft[1] + extent_of_slider[1]), 2)  # vertical
        pygame.draw.line(self.surface, (255, 255, 255), (self.topleft[0] + 2, self.topleft[1] + extent_of_slider[0]),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + extent_of_slider[0]), 2)  # horizontal
        pygame.draw.line(self.surface, (0, 0, 0),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + extent_of_slider[0]),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + extent_of_slider[1]), 2)  # vertical
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0], self.topleft[1] + extent_of_slider[1]),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + extent_of_slider[1]), 2)  # horizontal

        # draw up-arrow
        pygame.draw.line(self.surface, (255, 255, 255), (self.topleft[0] + 2, self.topleft[1] + 2),
                         (self.topleft[0] + 2, self.topleft[1] + self.width - 2), 2)  # vertical white
        pygame.draw.line(self.surface, (255, 255, 255), (self.topleft[0] + 2, self.topleft[1] + 2),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + 2), 2)  # horizontal white
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0] + self.width - 2, self.topleft[1] + 2),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + self.width - 2), 2)  # vertical black
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0] + 2, self.topleft[1] + self.width - 2),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + self.width - 2), 2)  # horizontal black
        pygame.draw.polygon(self.surface, (0, 0, 0), [(self.topleft[0] + self.width / 2, self.topleft[1] + 6),
                                                      (self.topleft[0] + 5, self.topleft[1] + 12),
                                                      (self.topleft[0] + self.width - 5, self.topleft[1] + 12)])

        # draw down-arrow
        pygame.draw.line(self.surface, (255, 255, 255),
                         (self.topleft[0] + 2, self.topleft[1] + 2 + self.length_of_bar_in_pixel - self.width),
                         (self.topleft[0] + 2, self.topleft[1] + self.length_of_bar_in_pixel - 2), 2)  # vertical white
        pygame.draw.line(self.surface, (255, 255, 255),
                         (self.topleft[0] + 2, self.topleft[1] + 2 + self.length_of_bar_in_pixel - self.width), (
                         self.topleft[0] + self.width - 2,
                         self.topleft[1] + 2 + self.length_of_bar_in_pixel - self.width), 2)  # horizontal white
        pygame.draw.line(self.surface, (0, 0, 0), (
        self.topleft[0] + self.width - 2, self.topleft[1] + 2 + self.length_of_bar_in_pixel - self.width),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + self.length_of_bar_in_pixel - 2),
                         2)  # vertical black
        pygame.draw.line(self.surface, (0, 0, 0),
                         (self.topleft[0] + 2, self.topleft[1] + self.length_of_bar_in_pixel - 2),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + self.length_of_bar_in_pixel - 2),
                         2)  # horizontal black
        pygame.draw.polygon(self.surface, (0, 0, 0),
                            [(self.topleft[0] + self.width / 2, self.topleft[1] - 6 + self.length_of_bar_in_pixel),
                             (self.topleft[0] + 5, self.topleft[1] - 12 + self.length_of_bar_in_pixel),
                             (self.topleft[0] + self.width - 5, self.topleft[1] - 12 + self.length_of_bar_in_pixel)])
    ...
    def activate(self, pos):
        ...
        if not self.unmovable:
            if pos[1] - self.topleft[1] < self.width:  # up-arrow
                ...
                self.position = int((self.range_of_values[1] - self.range_of_values[0]) * percentage_pos) + \
                                self.range_of_values[0]
...
class hscrollbar():
    def __init__(self, surface, function, topleft, length_of_bar_in_pixel, range_of_values, range_seen=None,
        ...
        if range_of_values[1] < range_of_values[0]:
            raise Exception(
                "the first entry in range_of_values must be smaller than the second: " + str(range_of_values))
        ...
        if start_position < range_of_values[0] or start_position > range_of_values[1]:
            raise Exception("start position must be within the range_of_values. It was " + str(
                start_position) + " and range_of_values were: " + str(range_of_values))
    ...
    def draw(self):
        ...
        # draw slider
        extent_of_slider = self.calculate_extent_of_slider()
        pygame.draw.line(self.surface, (255, 255, 255), (self.topleft[0] + extent_of_slider[0], self.topleft[1] + 2),
                         (self.topleft[0] + extent_of_slider[0], self.topleft[1] + self.width - 2), 2)  # vertical white
        pygame.draw.line(self.surface, (255, 255, 255), (self.topleft[0] + extent_of_slider[0], self.topleft[1] + 2),
                         (self.topleft[0] + extent_of_slider[1] - 2, self.topleft[1] + 2), 2)  # horizontal white
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0] + extent_of_slider[1] - 2, self.topleft[1] + 2),
                         (self.topleft[0] + extent_of_slider[1] - 2, self.topleft[1] + self.width - 2),
                         2)  # vertical black
        pygame.draw.line(self.surface, (0, 0, 0),
                         (self.topleft[0] + extent_of_slider[0], self.topleft[1] + self.width - 2),
                         (self.topleft[0] + extent_of_slider[1] - 2, self.topleft[1] + self.width - 2),
                         2)  # horizontal black
        ...
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0] + 2, self.topleft[1] + self.width - 2),
                         (self.topleft[0] + self.width - 2, self.topleft[1] + self.width - 2), 2)  # horizontal black
        pygame.draw.polygon(self.surface, (0, 0, 0), [(self.topleft[0] + 6, self.topleft[1] + self.width / 2),
                                                      (self.topleft[0] + 12, self.topleft[1] + 5),
                                                      (self.topleft[0] + 12, self.topleft[1] - 5 + self.width)])

        # draw right-arrow
        pygame.draw.line(self.surface, (255, 255, 255),
                         (self.topleft[0] + self.length_of_bar_in_pixel - self.width + 2, self.topleft[1] + 2),
                         (self.topleft[0] - 2 + self.length_of_bar_in_pixel, self.topleft[1] + 2),
                         2)  # horizontal white
        pygame.draw.line(self.surface, (255, 255, 255),
                         (self.topleft[0] + self.length_of_bar_in_pixel + 2 - self.width, self.topleft[1] + 2), (
                         self.topleft[0] + self.length_of_bar_in_pixel + 2 - self.width,
                         self.topleft[1] - 2 + self.width), 2)  # vertical white
        pygame.draw.line(self.surface, (0, 0, 0), (
        self.topleft[0] + self.length_of_bar_in_pixel - self.width + 2, self.topleft[1] - 2 + self.width),
                         (self.topleft[0] - 2 + self.length_of_bar_in_pixel, self.topleft[1] - 2 + self.width),
                         2)  # horizontal black
        pygame.draw.line(self.surface, (0, 0, 0),
                         (self.topleft[0] + self.length_of_bar_in_pixel - 2, self.topleft[1] + 2),
                         (self.topleft[0] + self.length_of_bar_in_pixel - 2, self.topleft[1] - 2 + self.width),
                         2)  # vertical black
        pygame.draw.polygon(self.surface, (0, 0, 0),
                            [(self.topleft[0] + self.length_of_bar_in_pixel - 6, self.topleft[1] + self.width / 2),
                             (self.topleft[0] + self.length_of_bar_in_pixel - 12, self.topleft[1] + 5),
                             (self.topleft[0] + self.length_of_bar_in_pixel - 12, self.topleft[1] - 5 + self.width)])
    ...
    def update_radiobuttons(self):
        for i, label in enumerate(self.labels):
            pygame.draw.circle(self.surface, (255, 255, 255),
                               (self.topleft[0] + 10, self.topleft[1] + self.textheight // 2 + self.textheight * i), 6)
            pygame.draw.circle(self.surface, (0, 0, 0),
                               (self.topleft[0] + 10, self.topleft[1] + self.textheight // 2 + self.textheight * i), 6,
                               1)
            if label == self.selected:
                pygame.draw.circle(self.surface, (0, 0, 0),
                                   (self.topleft[0] + 10, self.topleft[1] + self.textheight // 2 + self.textheight * i),
                                   4)
...
class button():
    ...
    def __init__(self, label, surface, function, function_parameter=None, topleft=(0, 0), fixed_size=None):
        ...
        if fixed_size is None:
            ...
            self.rect = pygame.Rect(topleft[0], topleft[1], labelsize[0] + 2 * self.padding,
                                    labelsize[1] + 2 * self.padding)
        else:
    ...
    def draw(self):
        pygame.draw.rect(self.surface, (212, 212, 212), self.rect)
        pygame.draw.rect(self.surface, (0, 0, 0), self.rect, 1)
        pygame.draw.line(self.surface, (255, 255, 255), (self.rect[0], self.rect[1]),
    ...
    def draw_pressed(self):
        ...
        pygame.draw.line(self.surface, (0, 0, 0), (self.rect[0] + 1, self.rect[1]),
                         (self.rect[0] + 1, self.rect[1] + self.rect[3] - 2))  # vertical
        pygame.draw.line(self.surface, (0, 0, 0), (self.rect[0], self.rect[1] + 1),
                         (self.rect[0] + self.rect[2] - 2, self.rect[1] + 1), 1)  # horizontal
        self.surface.blit(self.rendered_label, (self.rect[0] + self.padding, self.rect[1] + self.padding))
        pygame.display.flip()
        time.sleep(0.05)
        self.draw()
...
class togglebutton():
    ...
    def __init__(self, label, surface, function, function_parameter=None, topleft=(0, 0), fixed_size=None,
        ...
        if fixed_size is None:
            labelsize = self.rendered_label.get_size()
            self.rect = pygame.Rect(self.topleft[0], self.topleft[1], labelsize[0] + 2 * self.padding,
                                    labelsize[1] + 2 * self.padding)
        else:
    ...
    def draw_pressed(self):
        pygame.draw.rect(self.surface, (212, 212, 212), self.rect)
        pygame.draw.rect(self.surface, (0, 0, 0), self.rect, 1)
        pygame.draw.rect(self.surface, (112, 112, 112),
                         pygame.Rect(self.topleft[0], self.topleft[1], self.rect[2], self.rect[3]))
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0] + 1, self.topleft[1]),
                         (self.topleft[0] + 1, self.topleft[1] + self.rect[3] - 2))  # vertical
        pygame.draw.line(self.surface, (0, 0, 0), (self.topleft[0], self.topleft[1] + 1),
                         (self.topleft[0] + self.rect[2] + - 2, self.topleft[1] + 1), 1)  # horizontal
...
class fast_list():
    ...
    def create_fast_list(self):
        ...
        self.vscrollbar = vscrollbar(self.surface, self.scrolling, (self.rect[0] + self.rect[2] - 20, self.rect[1]),
                                     self.rect[3], (0, len(self.data)), range_seen=self.lines_visible, start_position=0,
        ...
        if event.pos[0] > self.rect[0] + self.rect[2] - 20:
            ...
            if self.title is not None:
                if event.pos[1] < self.rect[1] + self.text_height:
                    ...
                    self.receive_data(self.original_tabular_data, sort_by=sort_by_this_column,
                                      column_order=self.original_column_order, reverse_sort=self.reverse_sorting)
                    self.update_fast_list()
                    pygame.display.flip()
                    return

            index_selected = (event.pos[1] - 20 - self.rect[1] - self.top_frame_width) / self.text_height + \
                             self.interval[0]
            ...
            if 0 <= index_selected and index_selected < len(self.data):
                self.selected = self.data[int(index_selected)]
    ...
    def update_fast_list(self):
        ...
        if self.lines_visible >= len(self.data):
            ...
            for i in range(0, len(self.data)):
                ...
                self.surface.blit(rendered_dataline, (self.rect_for_main_list[0] + self.left_list_frame,
                                                      self.rect_for_main_list[
                                                          1] + i * self.text_height + self.top_frame_width))
                pygame.display.flip()

        # in the case where data can't fit on screen
        else:

            percentage_position = float(self.vscrollbar.position) / float(
                self.vscrollbar.range_of_values[1] - self.vscrollbar.range_of_values[0])
            ...
            for j, i in enumerate(self.interval):
                ...
                self.surface.blit(rendered_dataline, (self.rect_for_main_list[0] + self.left_list_frame,
                                                      self.rect_for_main_list[
                                                          1] + j * self.text_height + self.top_frame_width))
    ...
    def receive_data(self, data, sort_by="rownames", column_order=None, reverse_sort=False):
        ...
        if isinstance(data, list):
            ...
            if data == {}:
                ...
                try:
                    data[list(data.items())[0][0]].keys()
                except:
                    print(data)
                    raise Exception("The data given to fast_list did not follow standards. It has been printed above")
                original_columns = ["rownames"] + list(data[list(data.items())[0][0]].keys())
                if column_order is None:
                    column_order = original_columns
                else:
                    for column_name in column_order:
                        if column_name not in original_columns:
                            raise Exception("Received a column_order entry " + str(
                                column_name) + " that was not located in the data columns: " + str(original_columns))
                ...
                for max_letters_per_column_entry in max_letters_per_column:
                    max_letters_per_column[max_letters_per_column_entry] = max_letters_per_column[
                                                                               max_letters_per_column_entry] + 2

                # sorting the rows according to sort_by
                if sort_by not in column_order:
                    print(column_order)
                    raise Exception(
                        "The sort_by variable was not found in the column_order. Remember the rownames must also be present if needed")
                if sort_by == "rownames":
                    sorting_list = list(sorted(data.keys()))
                else:
                    temp_dict = {}
                ...
                for rowname in sorting_list:
                    ...
                    for column_entry in column_order:
                        ...
                        if isinstance(data_point_here, int) or isinstance(data_point_here, float):
