...
class gui():
    ...
    def __init__(self, right_side_surface, message_surface, action_surface, solar_system_object):
        ...
        self.subcommand_rect = pygame.Rect(command_box_left, subcommand_top, command_width,
        ...
        self.command_surface = right_side_surface.subsurface(
            pygame.Rect(0, command_top, command_width, subcommand_top - command_top))
        self.subcommand_surface = right_side_surface.subsurface(
            pygame.Rect(0, subcommand_top, command_width, global_variables.window_size[1] - subcommand_top))
        ...
        self.all_windows["base_and_firm_market_window"] = base_and_firm_market_window(solar_system_object,
                                                                                      action_surface)
                ...
                if self.active_window.rect.collidepoint(event.pos) == 1:
                    ...
                    if return_value is not None:
                        if return_value == "clear":
                            ...
                            print_dict = {"text": "DEBUGGING: unknown display mode passed to infobox",
                                          "type": "debugging"}
    ...
    def clear_screen(self):
        ...
        if sol.display_mode == "solar_system":
            self.action_surface.blit(
                sol.draw_solar_system(zoom_level=sol.solar_system_zoom, date_variable=sol.current_date,
                                      center_object=sol.current_planet.planet_name), (0, 0))
        elif sol.display_mode == "planetary":
            self.action_surface.blit(sol.current_planet.draw_entire_planet(sol.current_planet.eastern_inclination,
    ...
    def zoom_in(self, event):
        ...
        if sol.display_mode == "solar_system":
            ...
            surface = sol.draw_solar_system(zoom_level=sol.solar_system_zoom, date_variable=sol.current_date,
    ...
    def click_in_action_window(self, event):
        ...
        if sol.display_mode == "solar_system":
            ...
            if sol.build_base_mode:  # if we are in the special build base mode, there should be a base creation instead.
                ...
                if sphere_coordinates[0:19] == "transfer population" or isinstance(sphere_coordinates,
                                                                                   tuple) or sphere_coordinates == "space base":  # if the selection was correctly verified by check_base_position we send it back to the GUI for further processing
                ...
                areas_of_interest = sol.current_planet.areas_of_interest[(
                sol.current_planet.northern_inclination, sol.current_planet.eastern_inclination,
                sol.current_planet.projection_scaling)]
    ...
    def create_infobox(self):
        ...
        if self.solar_system_object_link.current_player is not None:
            capital_string = str(
                primitives.nicefy_numbers(int(self.solar_system_object_link.current_player.capital))) + " $"
    ...
    def create_commandbox(self):
        ...
        labels = ["Map overlays", "Planet shortcuts", "Company menu", "Base overview", "Technology", "Trade menu",
                  "File menu"]
        ...
        for i, label in enumerate(labels):
            self.command_buttons[label] = gui_components.button(label,
                                                                ...
                                                                fixed_size=(self.command_rect[2] - 20, 35),
                                                                topleft=(10, i * 40 + 10))
    ...
    def create_subcommandbox(self):
        ...
        for i, label in enumerate(self.buttonlinks):
            self.subcommand_buttons[label] = gui_components.button(self.buttonnicenames[i], self.subcommand_surface,
                                                                   self.subcommandbox_button_activate,
                                                                   function_parameter=label,
                                                                   fixed_size=(
                                                                   self.subcommand_surface.get_width() - 20, 35),
                                                                   topleft=(10, i * 40 + 10))
...
class message_bar():
    ...
    def create(self):
        ...
        range_here = list(range(0, len(self.solar_system_object_link.messages)))[::-1]
...
class navigation_window():
    ...
    def create(self):
        ...
        pygame.draw.line(self.action_surface, (255, 255, 255), (self.rect[0], self.rect[1]),
                         (self.rect[0] + self.rect[2], self.rect[1]))
        pygame.draw.line(self.action_surface, (255, 255, 255), (self.rect[0], self.rect[1]),
                         (self.rect[0], self.rect[1] + self.rect[3]))

        self.button_north = gui_components.button("N", self.action_surface, self.rotate_north,
                                                  topleft=(self.rect[0] + 70, self.rect[1] + 10), fixed_size=(50, 30))
        self.button_west = gui_components.button("W", self.action_surface, self.rotate_west,
                                                 topleft=(self.rect[0] + 20, self.rect[1] + 40), fixed_size=(50, 30))
        self.button_south = gui_components.button("S", self.action_surface, self.rotate_south,
                                                  topleft=(self.rect[0] + 70, self.rect[1] + 70), fixed_size=(50, 30))
        self.button_east = gui_components.button("E", self.action_surface, self.rotate_east,
                                                 topleft=(self.rect[0] + 120, self.rect[1] + 40), fixed_size=(50, 30))

        self.button_zoom_in = gui_components.button("Zoom in", self.action_surface, self.zoom_in,
                                                    topleft=(self.rect[0] + 10, self.rect[1] + 120),
                                                    fixed_size=(80, 30))
        self.button_zoom_out = gui_components.button("Zoom out", self.action_surface, self.zoom_out,
                                                     topleft=(self.rect[0] + 100, self.rect[1] + 120),
                                                     fixed_size=(80, 30))
...
class planet_jump_window():
    ...
    def planet_jump(self, planet_name, function_parameter):
        ...
        self.solar_system_object_link.display_mode = "planetary"
        surface = planet.draw_entire_planet(planet.eastern_inclination, planet.northern_inclination,
                                            planet.projection_scaling)
        self.action_surface.blit(surface, (0, 0))
        pygame.display.flip()
    ...
    def receive_click(self, event):
        ...
        if 0 <= index < len(self.button_labels):
            ...
            if selection in self.solar_system_object_link.planets.keys():
                ...
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING: The planet jump function asked to go to a non-recognised planet",
                                  "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
    ...
    def create(self):
        ...
        for i, button_label in enumerate(self.button_labels):
            self.buttons[button_label] = gui_components.button(button_label, self.action_surface, self.planet_jump,
...
class tech_window():
    ...
    def create(self):
        ...
        surface = sol.technology_tree.plot_total_tree(sol.technology_tree.vertex_dict, sol.technology_tree.zoomlevel,
                                                      center=sol.technology_tree.center)
        self.action_surface.blit(surface, (0, 0))
        pygame.display.flip()

    def receive_click(self, event):
        if self.solar_system_object_link.message_printing["debugging"]:
            print_dict = {"text": "DEBUGGING: tech window received a direct click. This should not be possible",
                          "type": "debugging"}
            self.solar_system_object_link.messages.append(print_dict)
    ...
    def create(self):
        ...
        for planet_instance in self.solar_system_object_link.planets.values():
            for base_instance in planet_instance.bases.values():
                ...
                data_here = {"Location": planet_instance.name, "Population": base_instance.population,
                             "For sale": for_sale}
    ...
    def receive_click(self, event):
        ...
        if event.button == 3:
            ...
            if base_selected is None:
                raise Exception("The base sought after (" + str(
                    self.fast_list.selected_name) + ") was not found in the base list of the solar_system_object_link")
...
class trade_window():
    ...
    def create(self):
        ...
        for planet_instance in self.solar_system_object_link.planets.values():
            for base_instance in planet_instance.bases.values():
                if base_instance.for_sale:
                    data_here = {"Type": "base",
                                 "Best price": "for auction (pop: " + str(base_instance.population) + ")",
                                 "For sale by": base_instance.owner.name, "for_sale_by_link": [base_instance.owner],
        ...
        for technology in self.solar_system_object_link.technology_tree.vertex_dict.values():
            if len(technology["for_sale_by"]) > 0:
                ...
                check_result = self.solar_system_object_link.technology_tree.check_technology_bid(
                    self.solar_system_object_link.current_player.known_technologies, technology)
                if check_result != "already known":  # only include if we don't already know it
                    ...
                    data_here = {"Type": type, "Best price": best_price, "For sale by": for_sale_by,
                                 "for_sale_by_link": for_sale_by_link, "object": technology}
        ...
        if len(asset_and_tech_data) == 0:
            ...
            warning = global_variables.standard_font.render("Nothing is for sale on the tech and asset market.", True,
                                                            (0, 0, 0))
            self.action_surface.blit(warning, (self.rect[0] + self.rect[2] / 2 - warning.get_width() / 2,
                                               self.rect[1] + self.rect[3] / 2 - warning.get_height() / 2))


        else:

            column_order = ["rownames", "Type", "Best price", "For sale by"]
            self.fast_list = gui_components.fast_list(self.action_surface, asset_and_tech_data, rect=self.rect,
    ...
    def process_bid(self, bid_on):
        ...
        if self.selections["type"] in ["technology", "advanced tech."]:
            ...
            check_result = self.solar_system_object_link.technology_tree.check_technology_bid(current_player_tech,
                                                                                              self.selections[
                                                                                                  "sale_object"])
            if check_result != "ok":
                print_dict = {"text": "Can not bid for " + self.selections["bid_on"] + " because it is " + check_result,
                              "type": "general gameplay info"}
                self.solar_system_object_link.messages.append(print_dict)
                return None
        ...
        if len(self.selections["for_sale_by"]) > 1:
            if self.selections["type"] not in ["technology", "advanced tech."]:
                raise Exception("A bid was made for a type: " + str(
                    self.selections["type"]) + " asset, with more than one seller. This should not be possible")
    ...
    def perform_bid(self, chosen_seller_name):
        ...
        if chosen_seller_name not in self.solar_system_object_link.companies.keys():
            print_dict = {"text": str(chosen_seller_name) + " was not found - perhaps it was shut down recently.",
                          "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
        #            print "We had an instance of an unknown name: " + str(chosen_seller_name)
            ...
            if self.selections["type"] == "base":
                ...
                instruction = global_variables.standard_font.render(
                    "Enter bid for " + self.selections["bid_on"] + " (pop:" + str(
                        self.selections["sale_object"].population) + ") with deadline " + str(
                        self.selections["sale_object"].for_sale_deadline), True, (0, 0, 0))
                ...
                for resource in potential_base.mining_opportunities:
                    ...
                    for trade_route in potential_base.trade_routes.values():
                        ...
                        try:
                            ...
                            if self.solar_system_object_link.message_printing["debugging"]:
                                print_dict = {
                                    "text": "DEBUGGING: When calculating bid, we did not find a market in neighbour " + str(
                                        neighbour.name), "type": "debugging"}
                                self.solar_system_object_link.messages.append(print_dict)
                        else:
            ...
            if current_player.capital > self.selections["price"]:
                if self.selections["type"] in ["technology", "advanced tech."]:
                    self.solar_system_object_link.current_player.known_technologies[self.selections["bid_on"]] = \
                    self.selections["sale_object"]
                    ...
                    print_dict = {"text": str(self.selections["bid_on"]) + " was bought for " + str(
                        self.selections["price"]) + " from " + str(chosen_seller.name), "type": "general gameplay info"}
                    self.solar_system_object_link.messages.append(print_dict)
                    ...
                    raise Exception(
                        "Unknown type: " + str(self.selections["type"]) + " asked for in the asset sales GUI")


            else:
                print_dict = {"text": current_player.name + " has a capital of " + str(
                    current_player.capital) + " and can't bid " + str(self.selections["price"]),
                              "type": "general gameplay info"}
                self.solar_system_object_link.messages.append(print_dict)
    ...
    def effectuate_base_bid(self, label, function_parameters):
        ...
        try:
            ...
            print_dict = {"text": "The bid: " + str(price_string) + " was not a number",
                          "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
            return None
        else:
        ...
        if 3 * price > self.solar_system_object_link.current_player.capital:
            print_dict = {
                "text": "The bid: " + str(price_string) + " was too expensive - more than 3 times capital is required",
                "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
            return None
        #        else:
        ...
        if self.selections["sale_object"].for_sale_deadline is not None:
            if self.selections["sale_object"].for_sale_deadline <= self.solar_system_object_link.current_date:
                print_dict = {"text": "You are too late for the bidding deadline, which was " + str(
                    self.selections["sale_object"].for_sale_deadline), "type": "general gameplay info"}
                self.solar_system_object_link.messages.append(print_dict)
                return "clear"
        #            else:
        ...
        print_dict = {"text": "You have bid " + str(price) + " for " + str(
            self.selections["bid_on"]) + " - the auction will run till: " + str(
            self.selections["sale_object"].for_sale_deadline), "type": "general gameplay info"}
        self.solar_system_object_link.messages.append(print_dict)
        return "clear"
    ...
    def create(self):
        ...
        for company_name in self.solar_system_object_link.companies:
            ...
            if len(company_name) > global_variables.max_letters_in_company_names:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {
                        "text": "DEBUGGING: Shortened " + str(company_name) + " to " + str(company_name[0:30]),
                        "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
    ...
    def receive_click(self, event):
        ...
        if event.button == 3:
            ...
            if self.fast_list.selected_name in self.solar_system_object_link.companies.keys():
                ...
                print_dict = {
                    "text": "DEBUGGING:  " + str(self.fast_list.selected_name) + " was not found in company database",
                    "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)
...
class file_window():
    ...
    def draw(self):
        ...
        self.button_list_now = sorted(self.button_structure[self.position].keys())
        ...
        for i, button_name in enumerate(self.button_list_now):
            if isinstance(self.button_structure[self.position][button_name], str):  # has a submenu
                self.button_instances_now[button_name] = gui_components.button(button_name,
                                                                               ...
                                                                               self.go_to_submenu,
                                                                               function_parameter=
                                                                               self.button_structure[self.position][
                                                                                   button_name],
                                                                               fixed_size=(self.rect[2] - 20, 35),
                                                                               topleft=(10 + self.rect[0],
                                                                                        i * 40 + 10 + self.rect[1])
                                                                               )
        ...
        if self.distribute_click_to_subwindow is None:
            if isinstance(event.pos[1], int):
                ...
                if index >= 0 and index < len(self.button_list_now):
                    button_pressed = self.button_list_now[int(index)]
            ...
            if event.button == 3:
                if self.position == "Load menu":
                    self.solar_system_object_link.load_solar_system(
                        os.path.join("savegames", self.distribute_click_to_subwindow.selected))
    ...
    def select_save_name(self, label, function_parameter):
        ...
        self.button_instances_now["Name box"] = gui_components.entry(self.action_surface,
                                                                     topleft=(
                                                                     10 + self.rect[0], 10 + 40 + self.rect[1]),
    ...
    def automation_settings(self, label, function_parameter):
        ...
        if self.solar_system_object_link.current_player is None:
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {"text": "DEBUGGING: Game is in simulation mode so no changes can be made",
                              "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)
        else:
            pygame.draw.rect(self.action_surface, (150, 150, 150), self.rect)
                ...
                self.button_instances_now[button_name] = gui_components.togglebutton(button_name,
                                                                                     ...
                                                                                     topleft=(10 + self.rect[0],
                                                                                              i * 40 + 10 + self.rect[
                                                                                                  1]),
                                                                                     pressed=
                                                                                     self.solar_system_object_link.current_player.automation_dict[
                                                                                         button_name]
                                                                                     )
    ...
    def change_automation(self, label, function_parameter):
        ...
        print_dict = {"text": "For " + self.solar_system_object_link.current_player.name + " the " + str(
            function_parameter) + " was changed from " + str(previous_setting) + " to " + str(not previous_setting),
                      "type": "general company info"}
        self.solar_system_object_link.messages.append(print_dict)
    ...
    def check_and_save_decision_variables(self):
            ...
            for row_index in range(0, self.window.rows - 1):  # don't count the last row with buttons
                if self.window.grid[(row_index, column_offset)] is not None:
                    ...
                    try:
                        ...
                        print_dict = {"text": "The value " + str(value) + " at " + str(name) + " is not integer",
                                      "type": "general gameplay info"}
                        self.solar_system_object_link.messages.append(print_dict)
                        self.manager.emit("update_infobox", None)
                    ...
                    if 1 <= value_as_int <= 100:
                        ...
                        print_dict = {
                            "text": "The value " + str(value) + " at " + str(name) + " is not between 1 and 100",
                            "type": "general gameplay info"}
                        self.solar_system_object_link.messages.append(print_dict)
                        self.manager.emit("update_infobox", None)
                        all_passed_check = False
        ...
        if all_passed_check:
            print_dict = {
                "text": "The decision matrix has been updated for " + self.solar_system_object_link.current_player.name,
                "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
            self.manager.emit("update_infobox", None)
            for column_offset in [0, 2]:
        ...
        for i, button_name in enumerate(button_names):
            ...
            self.button_instances_now[button_name] = gui_components.togglebutton(button_name,
                                                                                 ...
                                                                                 topleft=(10 + self.rect[0],
                                                                                          i * 40 + 10 + self.rect[1]),
                                                                                 pressed=
                                                                                 self.solar_system_object_link.message_printing[
                                                                                     button_name]
                                                                                 )
    ...
    def change_message_setting(self, label, function_parameter):
        ...
        if function_parameter not in self.solar_system_object_link.message_printing.keys():
            raise Exception(
                "The message type " + str(function_parameter) + " was not found in the message_printing dict")
        ...
        print_dict = {"text": "The show-settings for " + str(function_parameter) + " was changed from " + str(
            previous_setting) + " to " + str(not previous_setting), "type": "general gameplay info"}
        self.solar_system_object_link.messages.append(print_dict)
    ...
    def create(self):
        ...
        if base_selected is not None:
            ...
            for resource in base_selected.input_output_dict["input"]:
                base_population_dict["Input: " + resource] = {
                    "info": base_selected.input_output_dict["input"][resource]}
            ...
            self.fast_list = gui_components.fast_list(self.action_surface, base_population_dict, rect=self.rect,
                                                      column_order=["rownames", "info"])
        else:
            if self.solar_system_object_link.message_printing["debugging"]:
    ...
    def receive_click(self, event):
        ...
        if event.button == 3:
            ...
            if self.fast_list.selected_name in self.solar_system_object_link.companies.keys():
                ...
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING:  " + str(
                        self.fast_list.selected_name) + " was not found in company database", "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
...
class base_and_firm_market_window():
    ...
    def __init__(self, solar_system_object, action_surface):
        ...
        self.resource_selected = list(self.solar_system_object_link.trade_resources.items())[0][0]
        ...
        self.positional_database = {"bidding_mode": {},
                                    "non_bidding_mode": {}}  # can be filled with information about clicks that the graphs receive
    ...
    def create(self):
        ...
        if self.solar_system_object_link.display_mode == "base":
            ...
            raise Exception("The display mode " + str(
                self.solar_system_object_link.display_mode) + " is not supposed to show market data")
        ...
        if self.solar_system_object_link.display_mode == "firm":
            ...
            if isinstance(firm_selected, company.merchant):
                self.market_selection_buttons = gui_components.radiobuttons(
                    ...
                    function_parameter={"From: " + firm_selected.from_location.name: firm_selected.from_location,
                                        "To: " + firm_selected.to_location.name: firm_selected.to_location},
            ...
            raise Exception("Unknown display_mode: " + str(global_variables.display_mode))

        # Add an update button that allows for updates to be done
        try:
            self.market_selection_buttons.rect
        except:
            update_button_topleft = (self.rect[0] + 10, self.graph_buttons.rect[1] + self.graph_buttons.rect[3] + 20)
        else:
            update_button_topleft = (
            self.rect[0] + 10, self.market_selection_buttons.rect[1] + self.market_selection_buttons.rect[3] + 20)
        ...
        if firm_selected.name in self.solar_system_object_link.current_player.owned_firms.keys():
            self.bid_button = gui_components.togglebutton("Make market bid",
                                                          ...
                                                          topleft=(self.rect[0] + 10,
                                                                   self.update_button.rect[1] + self.update_button.rect[
                                                                       3] + 20),
                                                          fixed_size=None)
    ...
    def update_data_market_bids(self):
        ...
        if self.solar_system_object_link.current_planet.current_base is None:
            ...
            pygame.draw.line(market_analysis_surface, (50, 50, 50), (0, self.graph_rect[3] * 0.5 + 7),
                             (self.graph_rect[3], self.graph_rect[3] * 0.5 + 7), 3)
            pygame.draw.line(market_analysis_surface, (50, 50, 50), (0, self.graph_rect[3] * 0.5 - 7),
                             (self.graph_rect[3], self.graph_rect[3] * 0.5 - 7), 3)
            ...
            if len(prices) == 0:
                market_price_label = global_variables.standard_font.render("No " + resource + " on market", True,
                                                                           (0, 0, 0))
                ...
                if len(market["buy_offers"][resource]) == 0:
                    ...
                    market_price_description = "Only sell offers. Lowest is: " + "%.5g" % \
                                               market["sell_offers"][resource][0]["price"]
                    ...
                    market_price_description = "Only buy offers. Highest is: " + "%.5g" % \
                                               market["buy_offers"][resource][0]["price"]
                    market_analysis_surface.blit(buy, (0, self.graph_rect[3] - 15))

                else:
                    market_price_description = "Highest buy offer: " + "%.5g" % market["buy_offers"][resource][0][
                        "price"] + ". Lowest sell offer: " + "%.5g" % market["sell_offers"][resource][0]["price"]
                    market_price = (market["sell_offers"][resource][0]["price"] + market["buy_offers"][resource][0][
                        "price"]) * 0.5
                    market_analysis_surface.blit(sell, (0, 0))
                    market_analysis_surface.blit(buy, (0, self.graph_rect[3] - 15))
                market_price_label = global_variables.standard_font.render(market_price_description, True, (0, 0, 0))
                market_analysis_surface.blit(market_price_label,
                                             (self.graph_rect[2] / 100, self.graph_rect[3] * 0.5 - 4))

                if market_analysis_surface is None:
                    raise Exception(
                        "After plotting the mean price on the market_analysis_surface it suddenly became None")
                ...
                for i in range(0, len(prices)):
                    ...
                    if i == 0:
                        y_position_here = (self.graph_rect[3] - self.frame_size) - (
                                    ((prices[i] - ylim[0]) / (ylim[1] - ylim[0])) * plotting_area_height)
                    else:
                    ...
                    if i == len(prices) - 1:
                        ...
                        y_position_next = (self.graph_rect[3] - self.frame_size) - (
                                    ((prices[i + 1] - ylim[0]) / (ylim[1] - ylim[0])) * plotting_area_height)
                    ...
                    pygame.draw.line(market_analysis_surface, (50, 50, 50), (0, y_position_here),
                                     (x_length, y_position_here))
                    ...
                    self.positional_database["non_bidding_mode"][(left_border, top_border, width, height)] = {
                        "linkto": provider[i], "text": provider[i].name + ": " + "%.5g" % prices[i], "figure": (
                        (left_border, y_position_here + self.graph_rect[1]),
                        (left_border + width, y_position_here + self.graph_rect[1]))}

                # making x-axis scale
                x_axis_vertical_position_percent_of_frame = 0.9  # in percent of lower frame where 1 is at top of frame
                x_axis_vertical_position = self.graph_rect[3] - int(
                    self.frame_size * x_axis_vertical_position_percent_of_frame)
                pygame.draw.line(market_analysis_surface, (0, 0, 0), (0, x_axis_vertical_position),
                                 (self.graph_rect[2], x_axis_vertical_position), 3)
                pygame.draw.line(market_analysis_surface, (0, 0, 0), (0, self.graph_rect[3] - x_axis_vertical_position),
                                 (self.graph_rect[2], self.graph_rect[3] - x_axis_vertical_position), 3)
                pygame.draw.line(market_analysis_surface, (0, 0, 0), (0, x_axis_vertical_position),
                                 (0, self.graph_rect[3] - x_axis_vertical_position), 3)
                pygame.draw.line(market_analysis_surface, (0, 0, 0), (self.graph_rect[2], x_axis_vertical_position),
                                 (self.graph_rect[2], self.graph_rect[3] - x_axis_vertical_position), 3)
                max_x_axis_mark = 10 ** math.floor(math.log10(
                    max_quantity))  # the value of the maximal x-axis mark. If eg. max_quantity is 1021, the max_x_axis_mark is 10^4
                ...
                for i in range(0,
                               10):  # iterating "downwards" so to speak, because the x_mark_line will give the lineage of 10fold lower marks
                    ...
                    pygame.draw.line(market_analysis_surface, (0, 0, 0),
                                     (x_pos_here, x_axis_vertical_position + mark_height / 2),
                                     (x_pos_here, x_axis_vertical_position - mark_height / 2))
                    ...
                    market_analysis_surface.blit(x_mark_label, (
                    x_pos_here - self.graph_rect[2] / 100, x_axis_vertical_position + (mark_height)))
                    if market_analysis_surface is None:
                        raise Exception(
                            "At the end of the market_analysis_surface section, the surface had become None")
    ...
    def update_data_history(self):
        ...
        if len(market["transactions"][resource]) == 0:
            no_history_label = global_variables.standard_font.render("No " + resource + " sold on market", True,
                                                                     (0, 0, 0))
            ...
            history_surface = primitives.make_linear_y_axis(history_surface, self.frame_size, ylim,
                                                            self.solar_system_object_link, unit="price")
            history_surface = primitives.make_linear_x_axis(history_surface, self.frame_size, xlim,
                                                            solar_system_object_link=self.solar_system_object_link,
                                                            unit="date")
            ...
            for i in range(0, len(price)):
                x_position = int(
                    self.frame_size + ((self.graph_rect[2] - self.frame_size * 2) * (dates[i] - xlim[0])) / (
                                xlim[1] - xlim[0]))
                y_position = int(self.graph_rect[3] - (self.frame_size + (
                            (self.graph_rect[3] - self.frame_size * 2) * (price[i] - ylim[0]) / (ylim[1] - ylim[0]))))
                try:
                    dot_size = int(math.log10(quantity[i]))
                except:
                    dot_size = 1
                ...
                if seller[i] is not None and buyer[i] is not None:  # can happen with the empty startup transactions
                    self.positional_database["non_bidding_mode"][
                        (left_border, top_border, 2 * dot_size, 2 * dot_size)] = {"linkto": seller[i], "text": str(
                        seller[i].name) + " to " + str(buyer[i].name) + ": 10^" + str(dot_size) + "units", "figure": (
                    (dot_size), (x_position + self.graph_rect[0], y_position + self.graph_rect[1])),
                                                                                  "debug": left_border - dot_size}
    ...
    def make_manual_bid(self, initial_price=None, initial_quantity=None):
        ...
        if isinstance(firm_selected, company.merchant):
            ...
            raise Exception(
                "Oddly the resource " + str(resource) + " was neither found in the input or output of " + str(
                    firm_selected.name))
        ...
        if direction == "sell":  # in this case we are mostly interested in the stock
            ...
            if isinstance(firm_selected, company.merchant):
                if self.base_selected_for_merchant == firm_selected.from_location:
                    ...
                    raise Exception("The self.base_selected_for_merchant " + str(
                        self.base_selected_for_merchant.name) + " was neither in the from or the to_location of " + str(
                        firm_selected.name))
            else:
            ...
            # buy quantity
            if isinstance(firm_selected, company.merchant):
                quantity_max = max(quantity_max,
                                   (firm_selected.from_location.population + firm_selected.to_location.population) / 2)
            else:
        ...
        price_rect = pygame.Rect(self.graph_rect[0] + fixed_price_text.get_width(), self.graph_rect[1] + height_to_draw,
                                 self.graph_rect[2] - fixed_price_text.get_width(), fixed_price_text.get_height())
        ...
        quantity_rect = pygame.Rect(self.graph_rect[0] + fixed_quantity_text.get_width(),
                                    self.graph_rect[1] + height_to_draw,
                                    self.graph_rect[2] - fixed_quantity_text.get_width(),
                                    fixed_quantity_text.get_height())
        ...
        def quantity_execute(label, quantity_rect):
            ...
            variable_quantity_text = global_variables.standard_font.render(str(self.quantity_bar.position), True,
                                                                           (0, 0, 0))
            self.action_surface.blit(variable_quantity_text, (quantity_rect[0], quantity_rect[1]))
        ...
        self.quantity_bar = gui_components.hscrollbar(self.action_surface,
                                                      quantity_execute,
                                                      (self.graph_rect[0] + 10,
                                                       self.graph_rect[1] + height_to_draw + 20),
        ...
        self.ok_button = gui_components.button("ok",
                                               ...
                                               fixed_size=(100, 35),
                                               topleft=(self.graph_rect[0] + self.graph_rect[2] - 110,
                                                        self.graph_rect[1] + self.graph_rect[3] - 40)
                                               )
    ...
    def effectuate_market_bid(self, label, market):
        ...
        if direction == "buy":
            if price * quantity > firm_selected.owner.capital:
                print_dict = {"text": "Too low capital to bid for " + str(quantity) + " " + str(unit) + " of " + str(
                    resource) + " at price " + str(price), "type": "general gameplay info"}
                self.solar_system_object_link.messages.append(print_dict)
                return
            own_offer = {"resource": resource, "price": float(price), "buyer": firm_selected,
                         "name": firm_selected.name, "quantity": int(quantity),
                         "date": self.solar_system_object_link.current_date}
        elif direction == "sell":
            if isinstance(firm_selected, company.merchant):
            ...
            if quantity > quantity_available:
                print_dict = {
                    "text": firm_selected.name + " only has " + str(firm_selected.stock_dict[resource]) + " " + str(
                        unit) + " and can't offer " + str(quantity) + " for sale", "type": "general gameplay info"}
                self.solar_system_object_link.messages.append(print_dict)
                return
            own_offer = {"resource": resource, "price": float(price), "seller": firm_selected,
                         "name": firm_selected.name, "quantity": int(quantity),
                         "date": self.solar_system_object_link.current_date}
        else:
        ...
        print_dict = {"text": firm_selected.name + " succesfully made a " + str(direction) + " bid for " + str(
            quantity) + " " + str(unit) + " of " + str(resource) + " at price " + str(price),
                      "type": "general gameplay info"}
        self.solar_system_object_link.messages.append(print_dict)
            ...
            if self.graph_selected == "place bid mode":  # if we are in bidding mode
                ...
                if self.bidding_mode:  # if the graphs accept bids we start the bidding sections up
                    ...
                    if self.graph_selected == "market bids":
                        ...
                        if y_position > (
                                self.graph_rect[3] - 2 * self.frame_size) / 2 + self.blank_area_in_middle_height / 2:
                        ...
                        y_relative_position = ((self.graph_rect[3] - self.frame_size) - (
                                    position[1] - self.rect[1] - 21)) / float(self.graph_rect[3] - self.frame_size)
                        ...
                        if 0 < y_relative_position < 1:
                            if "price" in self.positional_database["bidding_mode"].keys():
                                ...
                                if price < 0:
                                    price = 0
                                    if self.solar_system_object_link.message_printing["debugging"]:
                                        print_dict = {"text": "Changed price from " + str(price) + " to 0",
                                                      "type": "debugging"}
                                        self.solar_system_object_link.messages.append(print_dict)
                            ...
                            if "quantity" in self.positional_database["bidding_mode"].keys():
                                ...
                                try:
                                    ...
                                    if self.solar_system_object_link.message_printing["debugging"]:
                                        print_dict = {"text": "DEBUGGING: no good selection of log10 max_qt",
                                                      "type": "debugging"}
                                        self.solar_system_object_link.messages.append(print_dict)
                                    quantity = None
                                else:
                    ...
                    if click_spot_result is not None:
                        if event.button == 3:
                            if not isinstance(click_spot_result[1]["linkto"],
                                              company.base):  # if it was a base there would be no point, since it would already in zoom
                                ...
                                elif isinstance(linkto, company.firm) or isinstance(linkto,
                                                                                    company.merchant) or isinstance(
                                        linkto, company.research):
                                    self.solar_system_object_link.display_mode = "firm"
                            ...
                            text = global_variables.standard_font_small.render(click_spot_result[1]["text"], True,
                                                                               (0, 0, 0))

                            text_position = (click_spot_result[1]["figure"][1][0] - text_size[0] / 2,
                                             click_spot_result[1]["figure"][1][1])
                            ...
                            if text_position[0] + text_size[0] > self.graph_rect[0] + self.graph_rect[2]:
                                text_position = (
                                self.graph_rect[0] + self.graph_rect[2] - text_size[0], text_position[1])

                            self.action_surface.blit(text, text_position)

                            if self.graph_selected == "market bids":
                                pygame.draw.line(self.action_surface, (100, 100, 255),
                                                 click_spot_result[1]["figure"][0], click_spot_result[1]["figure"][1])
                            elif self.graph_selected == "history":
                                pygame.draw.circle(self.action_surface, (100, 100, 255),
                                                   click_spot_result[1]["figure"][1], click_spot_result[1]["figure"][0])
                            else:
                                raise Exception("Unknown graph type " + self.graph_selected)
...
class base_build_menu():
    ...
    def create(self):
        ...
        buildoption_data["population transfer"][
            "input and output"] = "To existing or new base - costs depends on distance"
    ...
    def merchant_pick_resource(self, destination_name):
        ...
        for endpoint in trade_route_selected["endpoint_links"]:
            if endpoint != from_location:
                to_location = endpoint
        self.selections = {"to_location": to_location, "from_location": from_location,
                           "trade_route_selected": trade_route_selected}
                ...
                if firm_instance.from_location == self.selections["from_location"]:
                    if firm_instance.to_location == self.selections["to_location"]:
                        if firm_instance.resource == resource:
                            exists = True
        if exists:
            print_dict = {"text": "A merchant from " + str(self.selections["from_location"].name) + " to " + str(
                self.selections["to_location"].name) + " trading " + str(resource) + " does already exist",
                          "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
    ...
    def merchant_build(self, label, function_parameter):
        ...
        if 0 < len(name) <= global_variables.max_letters_in_company_names and unique:
            ...
            print_dict = {"text": "Built a merchant named " + str(name) + " between " + str(
                self.selections["from_location"].name) + " and " + str(
                self.selections["to_location"].name) + " trading in " + str(self.selections["resource"]),
                          "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
            self.selections = {}
            self.menu_position = "root"
        else:
            print_dict = {"text": "the selected name " + str(name) + " was too long. Has to be less than " + str(
                global_variables.max_letters_in_company_names) + " characters", "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
    ...
    def commodity_size_selection(self, firm_type):
        ...
        if existing_firm is None:
            ...
            self.text_receiver = gui_components.entry(self.action_surface,
                                                      topleft=(
                                                      self.rect[0] + 100, self.rect[1] + 90, self.rect[2] - 100,
                                                      self.rect[3] - 150),
                                                      width=300,
                                                      max_letters=global_variables.max_letters_in_company_names)
            self.text_receiver.active = True
            ...
            existing_firm_rendered_text = global_variables.standard_font.render(
                "An existing size " + str(existing_firm.size) + " firm of this type already owned here.", True,
                (0, 0, 0))
            self.action_surface.blit(existing_firm_rendered_text, (self.rect[0] + 130, self.rect[1] + 70))
            existing_firm_rendered_text = global_variables.standard_font.render(
                "Select new size and press ok if new size is required.", True, (0, 0, 0))
        ...
        def execute(label, technology):
            ...
            size_info = global_variables.standard_font_small.render("size: " + str(self.slider.position), True,
                                                                    (0, 0, 0))
                ...
                for resource in technology["input_output_dict"][put].keys():
                    ...
                    if resource in self.solar_system_object_link.mineral_resources + ["food"] and put == "output":
                        mining_opportunity = self.solar_system_object_link.current_planet.current_base.get_mining_opportunities(
                            self.solar_system_object_link.current_planet, resource)
                        ...
                        value_info = global_variables.standard_font_small.render(
                            resource + ": " + str(value) + " (location modifier: " + str(
                                round(mining_opportunity, 2)) + ")", True, (0, 0, 0))

                    else:
                        value = technology["input_output_dict"][put][resource]
                        value = value * self.slider.position
                        value_info = global_variables.standard_font_small.render(resource + ": " + str(value), True,
                                                                                 (0, 0, 0))
            ...
            self.ok_button = gui_components.button("ok",
                                                   ...
                                                   fixed_size=(100, 35),
                                                   topleft=(
                                                   self.rect[0] + self.rect[2] - 110, self.rect[1] + self.rect[3] - 40))
    ...
    def commodity_build_firm(self, label, existing_firm):
        ...
        if existing_firm is None:
            ...
            if not (0 < len(name) <= global_variables.max_letters_in_company_names and unique):
                print_dict = {"text": "the selected name " + str(
                    name) + " was too long and/or not unique. Has to be less than " + str(
                    global_variables.max_letters_in_company_names) + " characters", "type": "general gameplay info"}
                self.solar_system_object_link.messages.append(print_dict)
        ...
        if isinstance(name, str):
            print_dict = {"text": str(name) + ", a " + self.selections["firm_type"] + " firm of size " + str(
                size) + " was built at " + str(location.name) + " for " + str(owner.name),
                          "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
            ...
            raise Exception("The name used: " + str(name) + " was of class " + str(
                name.__class__) + " but should have been a string")
        return "clear"
        ...
        if company_selected is not None:
            ...
            for company_database_variable in company_selected.company_database:
                ...
                if len(company_database_variable_name_here) > global_variables.max_letters_in_company_names:
                    company_database_variable_name_here = company_database_variable_name_here[
                                                          0:global_variables.max_letters_in_company_names]
                company_ownership_dict["parameter: " + company_database_variable_name_here] = {
                    "info": str(company_selected.company_database[company_database_variable])}
...
class company_financial_info():
    ...
    def create(self):
        ...
        if len(company_selected.company_accounting) == 0:
            no_history_label = global_variables.standard_font.render("No history for " + company_selected.name, True,
                                                                     (0, 0, 0))
            ...
            blank_surface = primitives.make_linear_y_axis(blank_surface, self.frame_size, ylim,
                                                          solar_system_object_link=self.solar_system_object_link,
                                                          unit="capital")
            blank_surface = primitives.make_linear_x_axis(blank_surface, self.frame_size, xlim,
                                                          solar_system_object_link=self.solar_system_object_link,
                                                          unit="date")
            #            print "(self.graph_rect[2]-self.frame_size*2): " + str((self.graph_rect[2]-self.frame_size*2))
            for i in range(1, len(capital)):
                x1_position = int(
                    self.frame_size + ((self.rect[2] - self.frame_size * 2) * (dates[i - 1] - xlim[0])) / (
                                xlim[1] - xlim[0]))
                y1_position = int(self.rect[3] - (self.frame_size + (
                            (self.rect[3] - self.frame_size * 2) * (capital[i - 1] - ylim[0]) / (ylim[1] - ylim[0]))))
                x2_position = int(self.frame_size + ((self.rect[2] - self.frame_size * 2) * (dates[i] - xlim[0])) / (
                            xlim[1] - xlim[0]))
                y2_position = int(self.rect[3] - (self.frame_size + (
                            (self.rect[3] - self.frame_size * 2) * (capital[i] - ylim[0]) / (ylim[1] - ylim[0]))))
    ...
    def receive_click(self, event):
        ...
        if event.button == 3:
            ...
            if firm_selected is None:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "POSSIBLE DEBUGGING: - the firm asked for was of None type",
                                  "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
            ...
            for i, resource in enumerate(market["transactions"]):
                ...
                for j, transaction in enumerate(market["transactions"][resource]):
                    ...
                    if firm_selected.name in [buyer, seller]:
                        transactions[(i + 1) * (j + 1) * (k + 1)] = {"date": date, "buyer": buyer, "seller": seller,
...
class firm_process_info():
    ...
    def create(self):
        ...
        if firm_selected is not None:
            ...
            if isinstance(firm_selected, company.merchant):
                ...
                for direction_name in ["destination", "origin"]:
                    ...
                    for resource in [firm_selected.resource, firm_selected.transport_type]:
                        ...
                        if direction_name == "destination":
                            process_and_stock_dict[resource + " at " + direction_name]["current stock"] = \
                            firm_selected.to_stock_dict[resource]
                        else:
                            process_and_stock_dict[resource + " at " + direction_name]["current stock"] = \
                            firm_selected.from_stock_dict[resource]
            ...
            self.fast_list = gui_components.fast_list(self.action_surface, process_and_stock_dict, rect=self.rect,
                                                      column_order=["rownames", "direction", "rate", "current stock"])
...
class construct_base_menu():
    ...
    def new_base_ask_for_name(self, sphere_coordinates, give_length_warning=False):
        ...
        if self.solar_system_object_link.current_player != building_base.owner:
            print_dict = {"text": "Could not transfer population from " + str(
                building_base.name) + " because it is not owned by you.", "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
            pygame.mouse.set_cursor(*pygame.cursors.arrow)
            return
        ...
        if sphere_coordinates[0:19] == "transfer population":
            if sphere_coordinates[23:] in self.solar_system_object_link.current_planet.bases.keys():
                ...
                if self.solar_system_object_link.current_player != destination_base.owner:
                    print_dict = {"text": "Could not transfer population to " + str(
                        destination_base.name) + " because it is not owned by you.", "type": "general gameplay info"}
                    self.solar_system_object_link.messages.append(print_dict)
                    self.solar_system_object_link.build_base_mode = True
                    pygame.mouse.set_cursor(*pygame.cursors.diamond)
                    return
                ...
                raise Exception(
                    "The destination base " + str(sphere_coordinates[23:]) + "was not found in base dict of " + str(
                        self.solar_system_object_link.current_planet.name))
        else:
        ...
        if building_base.home_planet == self.solar_system_object_link.current_planet:  # intraplanetary
            if building_base.terrain_type != "Space" and sphere_coordinates != "space base":  # ground based
                ...
                distance = int(
                    building_base.home_planet.calculate_distance(sphere_coordinates, building_base.position_coordinate)[
                        0]) / 100
                ...
                if building_base.terrain_type != "Space" and sphere_coordinates == "space base":  # ground to space building - mostly depends on escape velocity
                    ...
                    distance = (
                                           self.solar_system_object_link.current_planet.gravity_at_surface * gravitational_constant) ** 2
        ...
        if destination_base is not None:
            ...
            if sphere_coordinates == "space base":
                ...
                location_description = "Building a base at (" + str(round(sphere_coordinates[0])) + "," + str(
                    round(sphere_coordinates[1])) + ") on " + self.solar_system_object_link.current_planet.name
        ...
        if destination_base is None:
            ...
            self.text_receiver = gui_components.entry(self.action_surface,
                                                      topleft=(
                                                      self.rect[0] + self.rect[2] / 2 - 100, self.rect[1] + 70),
                                                      width=200,
                                                      max_letters=global_variables.max_letters_in_company_names)
            self.text_receiver.active = True
        else:
        ...
        def population_execute(label, price_rect):
            ...
            text = primitives.nicefy_numbers(
                int(self.population_bar.position * self.pricing["steel_cost_per_person"])) + " steel"
            rendered_text = global_variables.standard_font.render(text, True, (0, 0, 0))
            self.action_surface.blit(rendered_text, (price_rect[0], price_rect[1] + 15))

            text = primitives.nicefy_numbers(
                int(self.population_bar.position * self.pricing["power_cost_per_person"])) + " power"
            rendered_text = global_variables.standard_font.render(text, True, (0, 0, 0))
            self.action_surface.blit(rendered_text, (price_rect[0], price_rect[1] + 30))

            text = primitives.nicefy_numbers(
                int(self.population_bar.position * self.pricing["transport_cost_per_person"])) + " " + transport_type
            rendered_text = global_variables.standard_font.render(text, True, (0, 0, 0))
            self.action_surface.blit(rendered_text, (price_rect[0], price_rect[1] + 45))

            text = primitives.nicefy_numbers(
                int(self.population_bar.position * self.pricing["electronics_cost_per_person"])) + " electronics"
            rendered_text = global_variables.standard_font.render(text, True, (0, 0, 0))
        ...
        description = global_variables.standard_font.render("Calculated on a cost-distance of " + str(int(distance)),
                                                            True, (0, 0, 0))
        ...
        self.cancel_button = gui_components.button("cancel",
                                                   ...
                                                   fixed_size=(100, 35),
                                                   topleft=(
                                                   self.rect[0] + self.rect[2] - 220, self.rect[1] + self.rect[3] - 40))
    ...
    def new_base_build(self, label, function_parameter):
        ...
        if self.text_receiver is not None:
            ...
            if sphere_coordinates == "space base":
                ...
                location_description = " at (" + str(round(sphere_coordinates[0])) + "," + str(
        ...
        if 0 < len(name) <= global_variables.max_letters_in_company_names and unique:
            ...
            print_dict = {
                "text": "The preparation of " + construction_name + location_description + " has started in " + str(
                    building_base.name), "type": "general gameplay info"}
            self.solar_system_object_link.messages.append(print_dict)
