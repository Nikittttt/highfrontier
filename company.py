...
class company:
    ...
    def calculate_company_database(self, model_company_database, standard_deviation=0):
        ...
        try:
            ...
            company_database_headers_file = open(os.path.join("data", "economy", "companies.txt"))
            ...
            if global_variables.company_database_headers[headers] == "int":
                if headers in model_company_database.keys():
                    if standard_deviation == 0:
                        ...
                        new_company_database[headers] = int(
                            math.fabs(random.gauss(model_company_database[headers], standard_deviation)))
                        while 1 > new_company_database[headers] or new_company_database[
                            headers] > 100:  # to make the deviation correct, even at the ends. I wonder if this is a good idea
        ...
        if self.picture_file != None:
            ...
            if number_of_files_to_pick_from == 0:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING: In get_company_background There are no JPGs in the given folder",
                                  "type": "debugging"}
    ...
    def evaluate_market(self):
        ...
        if self.automation_dict["Start research firms"]:
            ...
            function_to_choose = int(
                math.ceil(len(functions_to_choose_from) * (self.company_database["start_research_firms"] / 100.0)))
        ...
        if self.automation_dict["Commodities market (start commodity producing firms)"]:
            ...
            function_to_choose = int(math.ceil(
                len(functions_to_choose_from) * (self.company_database["evaluate_commodities_market"] / 100.0)))
            functions_to_choose_from[function_to_choose](self)
        ...
        if self.automation_dict["Asset market (buy bases and firms)"]:
            ...
            function_to_choose = int(
                math.ceil(len(functions_to_choose_from) * (self.company_database["evaluate_asset_market"] / 100.0)))
            functions_to_choose_from[function_to_choose](self)
        ...
        if self.automation_dict["Tech market (buy and sell technology)"]:
            ...
            function_to_choose = int(
                math.ceil(len(functions_to_choose_from) * (self.company_database["evaluate_tech_market"] / 100.0)))
            functions_to_choose_from[function_to_choose](self)
        ...
        if self.automation_dict["Transport market (start up merchant firms)"]:
            ...
            function_to_choose = int(math.ceil(
                len(functions_to_choose_from) * (self.company_database["evaluate_intercity_trade_market"] / 100.0)))
            functions_to_choose_from[function_to_choose](self)
        ...
        if self.automation_dict["Expand area of operation (search for new home cities)"]:
            ...
            function_to_choose = int(math.ceil(
                len(functions_to_choose_from) * (self.company_database["evaluate_expansion_opportunities"] / 100.0)))
            functions_to_choose_from[function_to_choose](self)
    ...
    def evaluate_firms(self):
        ...
        function_to_choose = int(
            math.ceil(len(functions_to_choose_from) * (self.company_database["evaluate_firms"] / 100.0)))
        functions_to_choose_from[function_to_choose](self)
    ...
    def pick_research(self):
        ...
        function_to_choose = int(
            math.ceil(len(functions_to_choose_from) * (self.company_database["pick_research"] / 100.0)))
        functions_to_choose_from[function_to_choose](self)
    ...
    def expand_home_cities(self, solar_system_object):
        ...
        #			print "made a list of " + str(len(potential_bases)) + " and picked from this " + str(new_home_city_name)
            ...
            if len(potential_base_names) > 0:
                ...
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {
                        "text": "DEBUGGING: The expand_home_cities function did not find any new home_cities for " + self.name + " which have the current home_cities: " + str(
                            self.home_cities), "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
    ...
    def close_company(self):
        ...
        for firm_name in self.owned_firms:
            ...
            if isinstance(firm_instance, base):
                ...
                firm_instance.for_sale_deadline = datetime.timedelta(
                    30 * 6) + self.solar_system_object_link.current_date
                print_dict = {
                    "text": firm_name + " is up for sale because the owning company " + self.name + " is going bankrupt",
                    "type": "base sales"}
                self.solar_system_object_link.messages.append(print_dict)
    ...
    def evaluate_self(self):
        ...
        if self.research >= self.target_technology_cost and self.target_technology is not None:
            ...
            try:
                self.target_technology["known_by"][self.name] = self
            except:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING: a firm discovered " + self.target_technology[
                        "technology_name"] + " which is known by " + str(
                        self.target_technology["known_by"]) + " this is probably a mistake", "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
            else:
                pass
            print_dict = {
                "text": self.name + " discovered " + self.target_technology["technology_name"] + " which cost " + str(
                    self.target_technology_cost) + " the remaining " + str(
                    research_rest) + " research points have been transferred to further research.",
                "type": "tech discovery"}
            self.solar_system_object_link.messages.append(print_dict)
            self.target_technology = None
        ...
        if self.target_technology is None:
            if self.automation_dict["Pick research (pick research automatically)"]:
                ...
                if self.research > 0:
                    print_dict = {"text": self.name + " needs to pick a technology. Go to the technology window",
        ...
        for firm_instance in self.owned_firms.values():
            if firm_instance.for_sale:
                if firm_instance.for_sale_deadline > self.solar_system_object_link.current_date:
                    bids = list(sorted(firm_instance.for_sale_bids.values()))
                    ...
                    while funding_not_ok:
                        ...
                        try:
                            ...
                            print_dict = {
                                "text": "Did not find a buyer for base " + firm_instance.name + " - extending deadline with half a year",
                                "type": "base sales"}
                            self.solar_system_object_link.messages.append(print_dict)
                    ...
                    if winning_company is not None:
                        print_dict = {
                            "text": firm_instance.name + " was for sale but was won by " + winning_company.name + " for the price of " + str(
                                int(winning_price)) + " buying from " + self.name, "type": "base sales"}
                        self.solar_system_object_link.messages.append(print_dict)
        ...
        if self.capital > 1000:  # FIXME check that it is a good idea to set this above zero
            ...
            if self.automation_dict["Evaluate firms (close problematic firms)"]:
                time_limit_of_firm_evaluation = int(((101 - self.company_database["evaluate_firms_often"]) / 100.0) * (
                        max_time_firm_and_market - min_time_firm_and_market) + min_time_firm_and_market)
                if (
                        self.solar_system_object_link.current_date - self.last_firm_evaluation).days > time_limit_of_firm_evaluation:
                    self.last_firm_evaluation = self.solar_system_object_link.current_date
                    self.evaluate_firms()

            # market evaluation
            time_limit_of_market_evaluation = int(((101 - self.company_database["evaluate_market_often"]) / 100.0) * (
                    max_time_firm_and_market - min_time_firm_and_market) + min_time_firm_and_market)
            if (
                    self.solar_system_object_link.current_date - self.last_market_evaluation).days > time_limit_of_market_evaluation:
                self.last_market_evaluation = self.solar_system_object_link.current_date
            ...
            if self.automation_dict["Supply bidding (initiate selling bids)"]:
                time_limit_of_supply_evaluation = int(
                    ((101 - self.company_database["evaluate_supply_often"]) / 100.0) * (
                            max_time_supply_and_demand - min_time_supply_and_demand) + min_time_supply_and_demand)
                if (
                        self.solar_system_object_link.current_date - self.last_supply_evaluation).days > time_limit_of_supply_evaluation:
                    self.last_supply_evaluation = self.solar_system_object_link.current_date
                    for firm_instance in self.owned_firms.values():
            ...
            if self.automation_dict["Demand bidding (initiate buying bids)"]:
                time_limit_of_demand_evaluation = int(
                    ((101 - self.company_database["evaluate_demand_often"]) / 100.0) * (
                            max_time_supply_and_demand - min_time_supply_and_demand) + min_time_supply_and_demand)
                if (
                        self.solar_system_object_link.current_date - self.last_demand_evaluation).days > time_limit_of_demand_evaluation:
                    self.last_demand_evaluation = self.solar_system_object_link.current_date
                    for firm_instance in self.owned_firms.values():
            ...
            for stock_exchange in stock_exchanges:
                ...
                for line in raw_read:
                    ...
                    for singleword in old_splitline:
                        singleword = singleword.capitalize()
                    ...
                    if len(splitline) > 1:
                        if len(splitline) == 2:
                            ...
                            pass
            word_list = {"first_words": first_words, "second_words": second_words, "third_words": third_words,
    ...
    def make_up_names(self, wordlist):
        ...
        if number_of_middle_words == 2:
            middle_words = str(second_words[random.randrange(0, len(second_words))]) + " " + str(
                third_words[random.randrange(0, len(third_words))])
    ...
    def change_firm_size(self, location, size, technology_name, name=None):
        ...
        if isinstance(size, float):
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {"text": "DEBUGGING: change_firm_size received a float " + str(size) + " -- correct this",
                              "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)
            size = int(size)
        ...
        # This is the reaction to a size that is not zero - ie. a "build something" or "change the size of something" order
            ...
            if existing_firm is None:  # start up a new one
                ...
                if technology_name == "research":
                    ...
                    new_firm = research(solar_system_object=self.solar_system_object_link, location=location, input_output_dict=input_output_dict, owner=self,  name=firm_name)
                    new_firm.size = size
                ...
                # if it is not we search for the process requested
                    ...
                    for direction in ["input", "output"]:
                        ...
                        for resource_processed in self.known_technologies[technology_name]["input_output_dict"][
                            direction]:
                            firm_specific_process[direction][resource_processed] = \
                                self.known_technologies[technology_name]["input_output_dict"][direction][
                                    resource_processed] * size
                    for other_keys in self.known_technologies[technology_name][
                        "input_output_dict"].keys():  # to include whatever might have been in the input_output_dict
                        if other_keys not in ["input", "output"]:
                            firm_specific_process[other_keys] = \
                                self.known_technologies[technology_name]["input_output_dict"][other_keys]
                    ...
                    if name is None:
                        ...
                        firm_name = name
                    new_firm = firm(self.solar_system_object_link, location, firm_specific_process, self, firm_name,
                                    technology_name)
                    ...
                    for resource in new_firm.input_output_dict["input"]:
                        if new_firm.input_output_dict["input"][resource] <= 0:
                            raise Exception(
                                "The " + firm_name + " just created had an input of " + resource + " of " + str(
                                    new_firm.input_output_dict["input"][resource]))
                ...
                if technology_name == "research":
                    ...
                    for direction in ["input", "output"]:
                        for resource in existing_firm.input_output_dict[direction]:
                            ...
                            existing_firm.input_output_dict[direction][resource] = (
                                                                                           existing_value * size) / original_size
                            existing_firm.size = size
                    ...
                    for resource in existing_firm.input_output_dict["input"]:
                        if existing_firm.input_output_dict["input"][resource] <= 0:
                            raise Exception(
                                "The " + existing_firm.name + " just size-changed had an input of " + resource + " of " + str(
                                    existing_firm.input_output_dict["input"][
                                        resource]) + " the requested size was " + str(
                                    size) + " and the original size was " + str(original_size))
...
class firm():
    def __init__(self, solar_system_object, location, input_output_dict, owner, name, technology_name):
        ...
        if not isinstance(location, base):
            raise Exception(
                self.name + " is a regular firm but received a location that was not a base: " + str(location))
        ...
        #						print_dict = {"text":"The " + resource + " is up for closing","type":"firm info"}
        #						self.solar_system_object_link.messages.append(print_dict)
    ...
    def get_firm_background(self):
        ...
        if self.picture_file != None:
            ...
            if number_of_files_to_pick_from == 0:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING: In get_firm_background There are no JPGs in the given folder",
                                  "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
            else:
                my_pick = random.randrange(0, number_of_files_to_pick_from)
    ...
    def calculate_demand_reaction(self):
        ...
        function_to_choose = int(math.ceil(
            len(functions_to_choose_from) * (self.owner.company_database["calculate_demand_reaction"] / 100.0)))
        functions_to_choose_from[function_to_choose](self)
    ...
    def calculate_supply_reaction(self):
        ...
        function_to_choose = int(math.ceil(
            len(functions_to_choose_from) * (self.owner.company_database["calculate_supply_reaction"] / 100.0)))
        functions_to_choose_from[function_to_choose](self)
    ...
    def make_market_bid(self, market, own_offer):
        ...
        if not (isinstance(own_offer["quantity"], int)):
            own_offer["quantity"] = int(own_offer["quantity"])
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {"text": "DEBUGGING: The quantity given in an offer from " + str(
                    self.name) + ", which is using " + str(self.decision_data["demand_function"]) + " and " + str(
                    self.decision_data["supply_function"]) + " is not an integer. Try to keep it as integers",
                              "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)

        if not isinstance(own_offer["price"], float):
            print_dict = {
                "text": "DEBUGGING: The price given in an offer from " + str(self.name) + ", which is using " + str(
                    self.decision_data["demand_function"]) + " and " + str(
                    self.decision_data["supply_function"]) + " is not a float. Try to keep it as floats",
                "type": "debugging"}
            if self.solar_system_object_link.message_printing["debugging"]:
                self.solar_system_object_link.messages.append(print_dict)
        ...
        if "seller" in own_offer.keys():
            ...
            if self.stock_dict[resource] < own_offer["quantity"]:
                ...
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING WARNING: adjusted " + resource + " sell offer from " + str(
                        self.name) + " to " + str(own_offer[
                                                      "quantity"]) + " because of lack of resources  - you should try to correct this from the calculate_supply functions",
                                  "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
            ...
            if self.owner.capital < (own_offer["price"] * own_offer["quantity"]):
                ...
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "DEBUGGING WARNING: adjusted buy offer from " + str(self.name) + " to " + str(
                        own_offer[
                            "quantity"]) + "because of lack of capital - you should try to correct this from the "
                                           "calculate_demand functions. The original price was " + str(
                        own_offer["price"]) + " and the capital was " + str(self.owner.capital), "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
        ...
        if own_offer["quantity"] < 0:
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {
                    "text": "DEBUGGING WARNING: The quantity " + str(own_offer["quantity"]) + " offered by " + str(
                        self.name) + "is not a positive amount. This should be corrected from market_decisions. It "
                                     "was set to 0 as a safeguard",
                    "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)
        ...
        while quantity_found < own_offer["quantity"]:
            ...
            if (opposite_bids[i]["price"] >= own_offer["price"] and type == "sell_offer") or (
                    opposite_bids[i]["price"] <= own_offer["price"] and type == "buy_offer"):
                ...
                if counterpart.owner.capital < (
                        opposite_bids[i]["quantity"] * opposite_bids[i]["price"]) and type == "sell_offer":
                    opposite_bids[i]["quantity"] = max(int(counterpart.owner.capital / opposite_bids[i]["price"]), 0)

                if opposite_bids[i]["quantity"] < 0:
                    if self.solar_system_object_link.message_printing["debugging"]:
                        print_dict = {
                            "text": "DEBUGGING WARNING: The quantity in an offer from " + counterpart.name + " was changed to " + str(
                                opposite_bids[i]["quantity"]) + " during a " + type + " from " + str(
                                self.name) + " regarding " + resource, "type": "debugging"}
                        self.solar_system_object_link.messages.append(print_dict)
        ...
        for offer_of_interest in offers_of_interest:
            ...
            if offer_of_interest["quantity"] < 0:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {
                        "text": "DEBUGGING WARNING: The quantity in an offer_of_interest from " + counterpart.name + " regarding " + resource + " was found to be " + str(
                            offer_of_interest["quantity"]) + ". " + str(
                            counterpart.name) + " has a calculate_supply_reaction parameter of " + str(
                            counterpart.owner.company_database[
                                "calculate_supply_reaction"]) + " and a calculate_demand_reaction parameter of " + str(
                            counterpart.owner.company_database["calculate_demand_reaction"]), "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
            ...
            if type == "sell_offer":
                counterpart = offer_of_interest["buyer"]
                counterpart.owner.capital = counterpart.owner.capital - offer_of_interest["price"] * offer_of_interest[
                    "quantity"]
                ...
                if self.owner == self.solar_system_object_link.current_player or counterpart.owner == self.solar_system_object_link.current_player:
                    print_dict = {"text": self.name + " sold " + str(offer_of_interest[
                                                                         "quantity"]) + " units of " + resource + " to " + counterpart.name + " for a price of " + str(
                        offer_of_interest["price"]), "type": "firm info"}
                    self.solar_system_object_link.messages.append(print_dict)

                transaction_report = {"seller": self, "buyer": counterpart, "price": offer_of_interest["price"],
                                      "quantity": offer_of_interest["quantity"], "date": own_offer["date"],
                                      "resource": resource}
            elif type == "buy_offer":
                counterpart = offer_of_interest["seller"]
                counterpart.owner.capital = counterpart.owner.capital + offer_of_interest["price"] * offer_of_interest[
                ...
                if self.owner == self.solar_system_object_link.current_player or counterpart.owner == self.solar_system_object_link.current_player:
                    print_dict = {
                        "text": str(self.name) + " bought " + str(offer_of_interest["quantity"]) + " units of " + str(
                            resource) + " from " + str(counterpart.name) + " for a price of " + str(
                            offer_of_interest["price"]), "type": "firm info"}
                    self.solar_system_object_link.messages.append(print_dict)

                transaction_report = {"seller": counterpart, "buyer": self, "price": offer_of_interest["price"],
                                      "quantity": offer_of_interest["quantity"], "date": own_offer["date"],
                                      "resource": resource}
            else:
                raise TypeError
            ...
            if transaction_report["quantity"] < 0:
                ...
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {
                        "text": "DEBUGGING WARNING: The quantity in a " + type + " of " + resource + " by " + self.name + " was found to be " + str(
                            transaction_report["quantity"]), "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
        ...
        if need_to_find_more and balance_of_findings != 0:
            if balance_of_findings >= 0:
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {
                        "text": "DEBUGGING MESSAGE: balance_of_findings was thought to be negative but was " + str(
                            balance_of_findings) + " for " + self.name, "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
            ...
            if len(competing_bids) == 0:
                ...
                for i, competing_bid in enumerate(competing_bids):
                    if (type == "sell_offer" and competing_bid["seller"].name == self.name) or (
                            type == "buy_offer" and competing_bid["buyer"].name == self.name):
                        competing_bids.pop(i)
                    if (competing_bid["price"] > own_offer[
                        "price"] and type == "sell_offer" and did_not_pass_price_range) or (
                            competing_bid["price"] < own_offer[
                        "price"] and type == "buy_offer" and did_not_pass_price_range):
                ...
                if own_offer["quantity"] < 0:
                    if self.solar_system_object_link.message_printing["debugging"]:
                        print_dict = {
                            "text": "DEBUGGING WARNING: The quantity in an offer from " + self.name + " somehow ended up being negative",
                            "type": "debugging"}
                        self.solar_system_object_link.messages.append(print_dict)
    ...
    def execute_stock_change(self, current_date):
		...
		"""
        try:
            self.last_consumption_date
        except:
            if (
                    current_date - self.solar_system_object_link.start_date).days > 100:  # because it is an error if there is no last_consumption_data
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "Small debugging warning. Did not find self.last_consumption_date for " + str(
                        self.name) + " when doing execute_stock_change(). self.solar_system_object_link.start_date was used but this should be corrected at some point",
                                  "type": "debugging"}
                    self.solar_system_object_link.messages.append(print_dict)
        ...
        if time_span_days > timeframe:
            ...
            while keep_calculating:
                ...
                for input_resource in self.input_output_dict["input"]:
                    new_stock_level[input_resource] = self.stock_dict[input_resource] - self.input_output_dict["input"][
                        input_resource] * number_of_rounds
            ...
            if time_span_days / timeframe > number_of_rounds and isinstance(self, base):
                if self.owner == self.solar_system_object_link.current_player:
                    print_dict = {"text": self.name + " is a base, it is starving, but it will continue to produce",
                                  "type": "base info"}
                    self.solar_system_object_link.messages.append(print_dict)
                number_of_rounds = time_span_days / timeframe
            ...
            if number_of_rounds > 0:
                ...
                for output_resource in self.input_output_dict["output"]:
                    ...
                    self.stock_dict[output_resource] = self.input_output_dict["output"][
                                                           output_resource] * number_of_rounds + self.stock_dict[
                                                           output_resource]

                # adding byproducts to the atmosphere
                for byproduct in self.input_output_dict["byproducts"]:
                    self.location.home_planet.change_gas_in_atmosphere(byproduct, self.input_output_dict["byproducts"][
                        byproduct] * number_of_rounds)
                    if self.owner == self.solar_system_object_link.current_player:
                        print_dict = {"text": "Because of " + self.name + " " + byproduct + " changed with " + str(
                            self.input_output_dict["byproducts"][byproduct] * number_of_rounds) + " units on " + str(
                            self.location.home_planet.name), "type": "climate"}
                        self.solar_system_object_link.messages.append(print_dict)

                for resource in self.input_output_dict["output"]:
                    if resource in self.solar_system_object_link.mineral_resources:
                        self.location.mining_performed[resource] = self.location.mining_performed[
                                                                       resource] + number_of_rounds * \
                                                                   self.input_output_dict["output"][resource]
                        if self.owner == self.solar_system_object_link.current_player:
                            print_dict = {"text": self.name + " mined " + primitives.nicefy_numbers(
                                int(number_of_rounds * self.input_output_dict["output"][resource])) + " on " + str(
                                self.location.home_planet.name), "type": "mining"}
                            self.solar_system_object_link.messages.append(print_dict)
...
class base(firm):
    ...
    def __init__(self, solar_system_object, base_name, home_planet, base_data, owner):
        ...
        self.mining_check_interval = random.randint(6000,
                                                    10000)  # just to keep it from cropping up everybody at the same time
        self.accounting = []
        ...
        self.input_output_dict = {"input": {"housing": 1 * self.population, "food": 1 * self.population,
                                            "consumer goods": 1 * self.population},
                                  "output": {"labor": 1 * self.population}, "timeframe": 30,
                                  "byproducts": {}}  # however this is highly changeable
    ...
    def calculate_demand_reaction(self):
        ...
        for resource in self.solar_system_object_link.trade_resources:
            if self.solar_system_object_link.trade_resources[resource]["demanded_by_base"]:
                ...
                if resource_level == 0:
                    ...
                    raise Exception(self.name + " had a negative amount (", str(resource_level),
                                    ") of " + str(resource))
        ...
        for resource in base_demand_dict:
            ...
            if quantity_wanted * price <= self.owner.capital and quantity_wanted > 0 and price > 0:
                buy_offer = {"resource": resource, "price": price, "buyer": self, "name": self.name,
                             "quantity": quantity_wanted, "date": self.solar_system_object_link.current_date}
            ...
            self.input_output_dict["input"][resource] = int(
                0.1 * (base_demand_dict[resource] / base_demand_dict["food"]) * self.population)
    ...
    def calculate_emigration(self):
        ...
        for trade_routes in self.trade_routes:
            ...
            if self.bitternes_of_base > neighbour.bitternes_of_base:
                percent_emigration_from_bitterness = (0.1 * (self.bitternes_of_base - neighbour.bitternes_of_base) / (
                        self.solar_system_object_link.bitterness_of_world[1] -
                        self.solar_system_object_link.bitterness_of_world[0])) / len(self.trade_routes)
                ...
                if self.is_on_dry_land == "almost":
                    ...
                    if self.owner == self.solar_system_object_link.current_player:
                        print_dict = {
                            "text": "the base: " + self.name + " is facing flooding and will experience a refuge surge",
                            "type": "base info"}
                        self.solar_system_object_link.messages.append(print_dict)
                ...
                people_leaving = int(
                    (percent_emigration_from_bitterness + percent_emigration_from_fear_of_flood) * self.population)
                if people_leaving > 0:
                    ...
                    if self.home_planet.solar_system_object_link.effectuate_migration:
                        if people_leaving < self.population:
                            ...
                            if self.owner == self.solar_system_object_link.current_player:
                                print_dict = {
                                    "text": "The base " + self.name + " has lost all of its population due to migration",
                                    "type": "base info"}
                                self.solar_system_object_link.messages.append(print_dict)
    ...
    def initialize_market(self):
        ...
        for trade_resource in self.solar_system_object_link.trade_resources:
            ...
            for i in range(1, number_of_startup_transactions):
                market["transactions"][trade_resource].append(
                    {"price": self.solar_system_object_link.trade_resources[trade_resource]["starting_price"],
                     "buyer": None, "seller": None, "quantity": random.randint(500, 1000),
                     "date": self.solar_system_object_link.start_date - datetime.timedelta(0)})
        return market
        ...
        if self.picture_file != None:
            ...
            if file_name in file_list:
                ...
                if number_of_files_to_pick_from == 0:
                    if self.solar_system_object_link.message_printing["debugging"]:
                        print_dict = {
                            "text": "DEBUGGING: in get_base_background there are no cities in the given folder",
                            "type": "debugging"}
                        self.solar_system_object_link.messages.append(print_dict)
    ...
    def calculate_trade_routes(self, planet):
        ...
        if self.terrain_type != "Space":  # only ground bases
            ...
            while len(self.trade_routes) < number_of_trade_routes:
                ...
                for other_base in planet.bases.values():
                    if other_base.base_name != self.base_name:
                        ...
                        if position_one[0] - search_length < position_two[0] < position_one[0] + search_length and \
                                position_one[1] - search_length < position_two[1] < position_one[1] + search_length:
                            if not other_base.base_name in self.trade_routes.keys():
                                ...
                                if len(other_base.trade_routes) < int(math.log10(other_base.population)):
                                    ...
                                    endpoints = [other_base.base_name,
                                                 self.base_name]  # try to remove this if you get the opportunity FIXME
                                    endpoint_links = [other_base, self]
                                    trade_route = {"distance": distance[0], "transport_type": transport_type,
                                                   "endpoints": endpoints,
                                                   "endpoint_links": endpoint_links}  # converting distance from list to float (has to be list see planet
            ...
            trade_route = {"distance": distance, "transport_type": transport_type, "endpoints": endpoints,
                           "endpoint_links": endpoint_links}  # converting distance from list to float (has to be list see planet
    ...
    def get_mining_opportunities(self, planet, resource):
        ...
        if resource == "food":
            ...
            max_distance_step_degrees_rounded = int(max_distance_step_degrees / distance_data["step_size"]) * \
                                                distance_data["step_size"]
            ...
            for i in range(0, max_distance_step_degrees_rounded + distance_data["step_size"],
                           distance_data["step_size"]):
            ...
            for distance in distance_matrix_here:
                ...
                for zero_position in distance_matrix_here[distance]:
                    ...
                    # print real_position
                    real_position = (int(real_position[0]), int(
                        real_position[1]))  # this has been shown to be necessary on linux computeres for some reason
                    resource_value = (100 * (resource_map.getpixel(real_position) - resource_extrema[0])) / (
                            resource_extrema[1] - resource_extrema[0])
        ...
        if self.mining_performed[resource] > 0:
            deposit_reduction = (self.mining_performed[
                                     resource] / 50000.0) / global_variables.mineral_deposit_size_multiplier
            ...
            if deposit_reduction > 1:
                ...
                self.mining_performed[
                    resource] = 50000.0 * global_variables.mineral_deposit_size_multiplier * deposit_reduction_rest
        ...
        for distance in resource_matrix:
            ...
            sum_of_resources_at_distance = (float(sum(resources_at_distance) / len(resources_at_distance))) * (
                    float(len(resource_matrix)) / float(distance_data["steps"]))
            ...
            sum_of_resources = sum_of_resources_at_distance / (
                    (distance + 2) * cost_addition_for_distance) + sum_of_resources
    ...
    def draw_mining_area(self, planet, overlay_image):
        ...
        max_distance_step_degrees = (float(range_km) / float(planet_circumference)) * 360.0
        max_distance_step_degrees_rounded = int(max_distance_step_degrees / distance_data["step_size"]) * distance_data[
            "step_size"]
        if max_distance_step_degrees_rounded >= (distance_data["step_size"] * distance_data["steps"]):
            # print "it is long"
            max_distance_step_degrees_rounded = distance_data["step_size"] * (distance_data["steps"] - 1)
            ...
            for pixels in distance_matrix_here[distance]:
                ...
                if not 0 < pixel_transposed[0] < overlay_image.size[0]:
                    if 0 > pixel_transposed[0]:
                        ...
                        pixel_transposed = (pixel_transposed[0] - overlay_image.size[0], pixel_transposed[1])
                #				print "pixel_transposed: " + str(pixel_transposed) + " overlay_image.size: " + str(overlay_image.size)
                breakpoint()
...
class base_construction(firm):
    ...
    def __init__(self, solar_system_object, input_output_dict, location, name, home_planet, base_to_be_build_data,
                 owner, size):
        self.name = name
        self.location = location
        self.owner = owner
            ...
            if all_materials_present:
                ...
                if self.base_to_be_build_data["population"] > self.location.population:
                    if self.base_to_be_build_data["population"] == 0:
                        print_dict = {"text": "Population of " + str(
                            self.location.name) + " was zero and the construction have been cancelled.",
                                      "type": "general gameplay info"}
                        self.solar_system_object_link.messages.append(print_dict)
                        ...
                        print_dict = {"text": "Population of " + str(
                            self.location.name) + " was less than required for " + self.name + ". Only + " + str(
                            self.base_to_be_build_data["population"]) + " people have been sent.",
                                      "type": "general gameplay info"}
                        self.solar_system_object_link.messages.append(print_dict)
                ...
                if self.base_to_be_build_data["destination_base"] is not None:
                    ...
                    print_dict = {
                        "text": str(self.base_to_be_build_data["population"]) + " people have moved from " + str(
                            self.location.name) + " to " + str(destination_base.name), "type": "general gameplay info"}
                    self.solar_system_object_link.messages.append(print_dict)
                ...
                # if this is a new base
                    ...
                    endpoints = [new_base.base_name,
                                 self.location.base_name]  # try to remove this if you get the opportunity FIXME
...
class research(tertiary):
    ...
    def __init__(self, solar_system_object, location, input_output_dict, owner, name):
        ...
        if not isinstance(location, base):
            breakpoint()
            raise Exception(
                self.name + " is a research firm but received a location that was not a base: " + str(location))
        self.location = location
        self.picture_file = None
        self.last_consumption_date = self.solar_system_object_link.current_date
        self.last_accounting = self.solar_system_object_link.current_date
    ...
    def execute_stock_change(self, current_date):
        ...
        try:
            ...
            if (
                    current_date - self.solar_system_object_link.start_date).days > 100:  # because it is an error if there is no last_consuptiom_data
                if self.solar_system_object_link.message_printing["debugging"]:
                    print_dict = {"text": "Small debugging warning. Did not find self.last_consumption_date for " + str(
                        self.name) + " when doing execute_stock_change(). self.solar_system_object_link.start_date was used but this should be corrected at some point",
                                  "type": "debugging"}
        ...
        if time_span_days > timeframe:
            ...
            if number_of_rounds > 0:
                if self.owner == self.solar_system_object_link.current_player:
                    print_dict = {"text": "for " + self.name + ", " + str(
                        number_of_rounds) + " research rounds were completed adding research to " + self.owner.name,
                                  "type": "tech discovery"}
                    self.solar_system_object_link.messages.append(print_dict)
...
class merchant(tertiary):
    ...
    def __init__(self, solar_system_object, from_location, to_location, input_output_dict, owner, name, transport_type,
                 distance, resource):
        self.name = name
        self.owner = owner
        self.solar_system_object_link = solar_system_object
        if not isinstance(from_location, base):
            raise Exception(self.name + " is a merchant firm but received a from_location that was not a base: " + str(
                from_location))
        ...
        if not isinstance(to_location, base):
            raise Exception(
                self.name + " is a merchant firm but received a to_location that was not a base: " + str(to_location))
    ...
    def execute_stock_change(self, current_date):
        ...
        transportation_capacity = int(
            math.floor((self.from_stock_dict[self.transport_type] * 1000.0) / float(self.distance)))
        ...
        if quantity_to_transport > 0:
            ...
            self.from_stock_dict[self.transport_type] = self.from_stock_dict[self.transport_type] - int(
                math.ceil((quantity_to_transport * self.distance) / 1000.0))

            if self.owner == self.solar_system_object_link.current_player:
                print_dict = {"text": self.name + " transported " + str(quantity_to_transport) + " " + str(
                    self.resource) + " from " + self.from_location.name + " to " + self.to_location.name + " using " + str(
                    int((quantity_to_transport * self.distance) / 1000.0)) + " units of transport",
                              "type": "tech discovery"}
                self.solar_system_object_link.messages.append(print_dict)
    ...
    def calculate_demand_reaction(self):
        ...
        function_to_choose = int(math.ceil(
            len(functions_to_choose_from) * (self.owner.company_database["calculate_intercity_demand"] / 100.0)))
        functions_to_choose_from[function_to_choose](self)
    ...
    def calculate_supply_reaction(self):
        ...
        function_to_choose = int(math.ceil(
            len(functions_to_choose_from) * (self.owner.company_database["calculate_intercity_supply"] / 100.0)))
        functions_to_choose_from[function_to_choose](self)
