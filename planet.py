...
class planet:
    ...
    def change_gas_in_atmosphere(self, gas, ton):
        ...
        gravity_at_surface_ratio = self.solar_system_object_link.planets[
                                       "earth"].gravity_at_surface / self.gravity_at_surface

        ton_per_pa_here = ton_per_pa_on_earth * gravity_at_surface_ratio / surface_area_ratio

        ton_per_pa_here = ton_per_pa_here / (
                    global_variables.gas_change_multiplier * 100000)  # for fine tuning in global variables. The number is to try to keep 100.0 a standard value.
        ...
        if str("athmospheric_" + gas) in self.planet_data.keys():
            ...
            print_dict = {"text": "added " + str(ton) + " " + str(
                gas) + " to " + self.name + " which made the partial pressure change from " + str(
                (before)) + " to " + str(getattr(self, "athmospheric_" + gas)), "type": "climate"}
            self.solar_system_object_link.messages.append(print_dict)
        else:
            raise Exception(self.name + " did not have a " + str(
                "athmospheric_" + gas) + " entry in the athmospheric_ - only " + str(self.planet_data.keys()))
    ...
    def read_pre_base_file(self, planet_name):
        ...
        if os.access(data_file_name, os.R_OK):
            ...
            for base_name in read_base_database:
                base_instance = company.base(self.solar_system_object_link, base_name, self,
                                             read_base_database[base_name], placeholder)
    ...
    def calculate_distance(self, position_a, position_b):
        ...
        if len(position_a) != len(position_b):
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {"text": "WARNING: The two lists given in calculate_distance() are not the same length",
                              "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)
    ...
    def calculate_all_distances(self):
        ...
        if os.access(os.path.join("pickledmiscellanous", "distances"), os.R_OK):
            ...
            #with open('pickledmiscellanous\\distances1', 'rb') as infile:
            #    content = infile.read()
            #with open('pickledmiscellanous\\distances', 'wb') as output:
            #    for line in content.splitlines():
            #        output.write(line + str.encode('\n'))

            with open('pickledmiscellanous\\distances', "rb") as f:
        ...
        #
            ...
            for y1 in range(45):
                ...
                for x2 in range(90):
                    for y2 in range(45):
                        ...
                        distance_km = self.calculate_distance(((x1 * 4) - 180, (y1 * 4) - 90),
                                                              ((x2 * 4) - 180, (y2 * 4) - 90))
            ...
            print(
                "Did not find calculate_all_distances file. Performed calculations and saved them as distances in folder pickledmiscellanous")
    ...
    def pickle_all_projections(self):
        ...
        for projection_scaling in (45, 90, 180, 360):
            for eastern_inclination in (-150, -120, -90, -60, -30, 0, 30, 60, 90, 120, 150, 180):
                for northern_inclination in (-90, -60, -30, 0, 30, 60, 90):
                    pickle_file_name = str(self.planet_name) + "_" + str(projection_scaling) + "_zoom_" + str(
                        northern_inclination) + "_NS_" + str(eastern_inclination) + "_EW.jpg"
                    ...
                    if os.access(pickle_file_name_and_path, os.R_OK):
                        ...
                        print(str((eastern_inclination, northern_inclination,
                                   projection_scaling)) + " was not found - calculating")
                        self.load_for_drawing()
    ...
    def change_water_level(self, new_water_level):
        ...
        if new_water_level > len(table_of_topology):
            ...
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {
                    "text": "The planet " + str(self.planet_name) + " has reached its max water level of " + str(
                        len(table_of_topology)), "type": "debugging"}
                self.solar_system_object_link.messages.append(print_dict)
        ...
        for base in self.bases:
            if self.bases[base].terrain_type != "Space":
                ...
                position_y_pixel = int(
                    self.action_layer.size[1] - ((position_y_degrees + 90.0) / 180.0) * self.action_layer.size[1])
    ...
    def sphere_to_plane_total(self, sphere_coordinates, eastern_inclination, northern_inclination, projection_scaling):
        ...
        if self.projection_scaling <= 360:  # for the round world projection
            ...
            startup_string = "proj +proj=ortho +ellps=sphere +lon_0=" + str(eastern_inclination) + " +lat_0=" + str(
                northern_inclination)
            try:
                proj = subprocess.Popen(startup_string, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
            except:
                raise Exception(
                    "Calling the subprocess proj did not work. On windows machines that is weird. On unix-based machines it probably means that you should install proj 4.6. Check your local repository or google for proj + cartographic")
            else:
                pass
            stdout_text = proj.communicate(bytes(communication_string,encoding='utf8'))
            ...
            for i in range(0, len(stdout_text)):
                if stdout_text[i].find("*") != -1:
                    ...
                    x_proj = (x_raw / 6370997) * (projection_scaling * 0.5) + (
                                projection_scaling * 0.5)  # where 6370997 is the constant of the proj program
            ...
            for sphere_coordinate in sphere_coordinates:
                if (west_border < sphere_coordinate[0] < east_border) and (
                        south_border < sphere_coordinate[1] < north_border):
                    x_proj_position = ((sphere_coordinate[0] - west_border) / east_west_span) * window_size[0]
                    y_proj_position = window_size[1] - (
                                ((sphere_coordinate[1] - south_border) / north_south_span) * window_size[1])
    ...
    def plane_to_sphere_total(self, eastern_inclination, northern_inclination, projection_scaling,
        ...
        startup_string = "proj -I +proj=ortho +ellps=sphere +lat_0=" + str(-rotation_coordinates[1]) + " +lon_0=" + str(
        ...
        if projection_scaling <= 360:  # for the round world projection
            if isinstance(given_coordinates, list) or isinstance(given_coordinates, tuple):
                ...
                for i in range(0, (projection_dim[0] * projection_dim[1])):
                    projection_coordinates.append((i - (i / projection_dim[0]) * projection_dim[0], i / projection_dim[
                        1]))  # testing to see if the integer round works to my advantage
            ...
            if isinstance(given_coordinates, list) or isinstance(given_coordinates, tuple):
                ...
                for proj_position in given_coordinates:
                    ...
                    y_sphere_position = ((float(window_size[1]) - float(proj_position[1])) / window_size[
                        1]) * north_south_span + south_border
    ...
    def calculate_resource_map(self, resource_type):
        ...
        try:
            self.resource_maps[resource_type]
        except:
            resource_file_name_and_path = os.path.join("images", "planet", "nonrenewable materials map", resource_type,
                                                       str(self.planet_name + ".png"))
            if os.access(resource_file_name_and_path, os.R_OK):
                ...
                try:
                    self.planet_data[resource_type_in_database]
                except:
                    resource_level = 1
                    print("The resource_level for " + str(
                        resource_type_in_database) + " was not found in planet_database so the background value was set to 1")
                else:
                ...
                if self.planet_type != "gasplanet":
                    for y in range(0, 45):
                        for x in range(0, 90):
                            pixel = (random.randint(0, 255) + math.sin(
                                (x / 90.0) * 2 * math.pi + x_offset * 90 + y_skewing * (
                                            y / 7.0)) * 100 * x_band_clearness + math.sin(
                                (y / 90.0) * 2 * math.pi + y_offset * 90 + x_skewing * (
                                            y / 7.0)) * 100 * y_band_clearness) * resource_level
                            lut.append(pixel)
                else:
                    for y in range(0, 45):
                        for x in range(0, 90):
    ...
    def draw_overlay_map(self, eastern_inclination, northern_inclination, projection_scaling, resource_type):
        ...
        surface = self.draw_image(eastern_inclination=eastern_inclination, northern_inclination=northern_inclination,
                                  projection_scaling=projection_scaling, fast_rendering=True, image=overlay_image)

        return surface

    def draw_image(self, eastern_inclination, northern_inclination, projection_scaling, fast_rendering=False,
                   image=None, plane_to_sphere=None):
        """
        ...
        if projection_scaling <= 360:
            if (northern_inclination, eastern_inclination,
                projection_scaling) in self.pre_drawn_surfaces.keys() and check_memory:
                ...
                if plane_to_sphere is None:  # in this case we load the standard translation maps
                    #for i in ['0','30','-30','60','-60','90','-90']:
                    #    for j in ['45','90','180','360']:
                    #        with open(os.path.join('pickledprojections',"projection_" + str(i) + "_NS_" + str(j) + "_zoom (2)"), 'rb') as infile:
                    #            content = infile.read()
                    #        with open(os.path.join('pickledprojections',"projection_" + str(i) + "_NS_" + str(j) + "_zoom"), 'wb') as output:
                    #            for line in content.splitlines():
                    #                output.write(line + str.encode('\n'))

                    file_name = "projection_" + str(-northern_inclination) + "_NS_" + str(projection_scaling) + "_zoom"
                    file = open(os.path.join("pickledprojections", file_name), "rb")
                    #breakpoint()
                    ...
                    if (sphere_coordinate == "space"):
                        ...
                        if eastern_inclination != 0:
                            if sphere_coordinate[0] + eastern_inclination < 180:
                                ...
                                sphere_coordinate = (
                                sphere_coordinate[0] + eastern_inclination - 360, sphere_coordinate[1])

                        image_coordinate = ((sphere_coordinate[0] * image.size[0] + image.size[0] * 180) / 360,
                                            (sphere_coordinate[1] * image.size[1] + image.size[1] * 90) / 180)
                        index_first = int(math.floor(image_coordinate[0])) * index + int(
                            math.floor(image_coordinate[1])) * image.size[0] * index
                        new_image_string = new_image_string + str(image_string[index_first:(index_first + index)])
                ...
                if len(new_image_string) != (projection_scaling ** 2) * index:
                    if len(new_image_string) < (projection_scaling ** 2) * index:  # too short
                        ...
                        if missing > 5:
                            print("There are " + str(
                                missing) + " pixels missing. They have been added to the end of the image")
                        for i in range(0, missing):
                            new_image_string = new_image_string + "\x00"
                    if len(new_image_string) > (projection_scaling ** 2) * index:  # too long
                        print(
                            "the image string in the make_image_string function is too long!")  # perhaps a better error handling should be added
                breakpoint()
                surface = pygame.image.fromstring(bytes(new_image_string,encoding='utf-8'), [projection_scaling, projection_scaling], mode)
            ...
            north_border = int(
                max_north_center + (((northern_inclination - 60.0) / - 30.0)) * (north_south_span / 4.0) - (
                            window_size[1] / 2.0))
            south_border = int(
                max_north_center + (((northern_inclination - 60.0) / - 30.0)) * (north_south_span / 4.0) + (
                            window_size[1] / 2.0))
    ...
    def draw_entire_planet(self, eastern_inclination, northern_inclination, projection_scaling, fast_rendering=True):
        ...
        if self.planet_display_mode == "visible light":
            ...
            surface = self.draw_overlay_map(eastern_inclination, northern_inclination, projection_scaling,
                                            self.planet_display_mode)
        ...
        if self.planet_display_mode in ["visible light", "trade network"]:
            try:
                ...
                if (northern_inclination, eastern_inclination, projection_scaling) in self.pre_drawn_action_layers:
                    rgba_action_surface = self.pre_drawn_action_layers[
                        (northern_inclination, eastern_inclination, projection_scaling)]
                    surface.blit(rgba_action_surface, (0, 0))
                else:
                    rgba_action = self.convert_to_rgba(self.action_layer)
                    rgba_action_surface = self.draw_image(eastern_inclination, northern_inclination, projection_scaling,
                                                          fast_rendering=True, image=rgba_action)
                    surface.blit(rgba_action_surface, (0, 0))
                    self.pre_drawn_action_layers[
                        (northern_inclination, eastern_inclination, projection_scaling)] = rgba_action_surface

        surface = self.draw_bases(surface, eastern_inclination, northern_inclination, projection_scaling)

        # if self.planet_name == "mars":

        blackscreen.blit(surface, ((global_variables.window_size[0] - self.projection_dim[0]) / 2,
                                   (global_variables.window_size[1] - self.projection_dim[1]) / 2))
        surface = blackscreen
    ...
    def calculate_base_positions(self, eastern_inclination, northern_inclination, projection_scaling):
        ...
        """
        if (northern_inclination, eastern_inclination, projection_scaling) in self.base_positions.keys() and sorted(
                self.base_positions[(northern_inclination, eastern_inclination, projection_scaling)].keys()) == sorted(
                self.bases.keys()):
            ...
            if projection_scaling <= 360:  # for the round world projection
                ...
                for base in self.bases:
                    if self.bases[base].terrain_type == "Space":  # for space stations
                        ...
                        sphere_position = (
                        self.bases[base].position_coordinate[0], self.bases[base].position_coordinate[1])
                ...
                plane_coordinates = self.sphere_to_plane_total(sphere_coordinates, eastern_inclination,
                                                               northern_inclination, projection_scaling)
                reverse_plane_coordinates = self.sphere_to_plane_total(reverse_sphere_coordinates, eastern_inclination,
                                                                       northern_inclination, projection_scaling)

                for i in range(0, len(base_names)):
                    if plane_coordinates[i] != "Not seen":
                        base_positions_here[base_names[i]] = (
                        int(plane_coordinates[i][0]), int(plane_coordinates[i][1]))
                        absolute_position = (plane_coordinates[i][0] + (window_size[0] / 2) - (projection_scaling / 2),
                                             plane_coordinates[i][1] + (window_size[1] / 2) - (projection_scaling / 2))
                        areas_of_interest_here[(absolute_position[0] - 1, absolute_position[1] - 1, 2, 2)] = base_names[
                            i]
                    else:
                        ...
                        edge_position = (0.5 * projection_scaling * (1 + math.cos(angle)),
                                         projection_scaling - 0.5 * projection_scaling * (1 + math.sin(angle)))
                ...
                for base in self.bases:
                    if self.bases[base].terrain_type != "Space":
                        sphere_position = (
                        self.bases[base].position_coordinate[0], self.bases[base].position_coordinate[1])
                        if (west_border < sphere_position[0] < east_border) and (
                                south_border < sphere_position[1] < north_border):
                            x_proj_position = ((sphere_position[0] - west_border) / east_west_span) * window_size[0]
                            y_proj_position = window_size[1] - (
                                        ((sphere_position[1] - south_border) / north_south_span) * window_size[1])
            ...
            self.areas_of_interest[
                (northern_inclination, eastern_inclination, projection_scaling)] = areas_of_interest_here
    ...
    def kill_a_base(self, base_name):
        try:
            ...
            try:
                dying_base.owner.owned_firms[base_name]
            except:
                print("Didn't find " + base_name + " in owned firms of " + str(dying_base.owner.name))
            else:
            ...
            try:
                ...
                print("Didn't find " + base_name + " in bases of " + str(self.name))
            else:
    ...
    def check_base_position(self, projection_position):
        ...
        collision_test_result = click_spot.collidedict(
            self.areas_of_interest[(self.northern_inclination, self.eastern_inclination, self.projection_scaling)])
        if collision_test_result != None:
            return "transfer population to " + collision_test_result[1]

        if self.projection_scaling <= 360:  # for the round world projection
            transposed_projection_position = (
            projection_position[0] - global_variables.window_size[0] / 2 + self.projection_scaling / 2,
            projection_position[1] - global_variables.window_size[1] / 2 + self.projection_scaling / 2)
            proj_to_sphere_coordinates = self.plane_to_sphere_total(self.eastern_inclination, self.northern_inclination,
            ...
            y_sphere_position = -((((float(projection_position[1]) - window_size[1]) / window_size[
                1]) * north_south_span) - south_border)
        ...
        if isinstance(sphere_coordinates, tuple):
            ...
            square_size_degrees = int(math.ceil(10 * (
                        12756 / self.planet_diameter_km)))  # this is first used, to quickly sort away bases far away - the real check is truly circular. It is normalized to earth radius.
            ...
            for base_instance in self.bases.values():
                if base_instance.terrain_type != "Space":
                    if abs(base_instance.position_coordinate[0] - sphere_coordinates[0]) < square_size_degrees and abs(
                            base_instance.position_coordinate[1] - sphere_coordinates[1]) < square_size_degrees:
                        distance = self.calculate_distance(base_instance.position_coordinate, sphere_coordinates)
                        if distance[0] < radius_size:
                            print_dict = {"text": "The new position is less than " + str(
                                radius_size) + " km to " + base_instance.name + " it is " + str(distance[0]),
            ...
            if not_too_close:
                ...
                position_y_pixel = int(
                    self.action_layer.size[1] - ((sphere_coordinates[1] + 90.0) / 180.0) * self.action_layer.size[1])
                pixel_color = self.action_layer.getpixel((position_x_pixel, position_y_pixel))
        ...
        for base in base_positions:
            if base_positions[base][0] not in ["Space", "Not seen"]:
                if self.bases[base].is_on_dry_land == "almost":
                    ...
                    pygame.draw.circle(surface, (0, 0, 0), base_positions[base], int(inner_circle_radius))
                ...
                if self.current_base is not None:
                    if self.current_base.name == base:
                        pygame.draw.circle(surface, (255, 255, 255), base_positions[base], int(inner_circle_radius))
                ...
                for other_base in self.bases[base].trade_routes:
                    if other_base in base_positions.keys():
                        if base_positions[other_base][0] == "Not seen":
                            pygame.draw.line(surface, (155, 155, 155), base_positions[base],
                                             base_positions[other_base][1])
    ...
    def draw_space_stations(self, surface, eastern_inclination, northern_inclination, projection_scaling):
        ...
        if projection_scaling <= 180:
            ...
            relative_semi_major_axis = (
                                           self.planet_diameter_km) / km_per_pixels  # space stations are found on planet diameter from center of planet (ie, on planet-radius of height)
                ...
                if self.bases[space_station_name].terrain_type == "Space":
                    ...
                    if not 17 < orbit_position_index < 33 or northern_inclination != 0:
                        ...
                        if projection_scaling == 90:
                            space_station_label = global_variables.standard_font.render(space_station.name, True,
                                                                                        (255, 255, 255))
                            surface.blit(space_station_label, space_station_position)
                        if projection_scaling == 45:
                            space_station_label = global_variables.standard_font_small.render(space_station.name, True,
                                                                                              (255, 255, 255))
                            surface.blit(space_station_label, space_station_position)

                        viewpoint = (northern_inclination, eastern_inclination, projection_scaling)
                        self.areas_of_interest[viewpoint][
                            (space_station_position[0] - 1, space_station_position[1], 2, 2)] = space_station.name
    ...
    def explode(self, per_hit_intensity, number_of_hits, bases_involved, mainscreen):
        ...
        ratio = 0.25 * (float(per_hit_intensity) / 100.0) * (float(self.projection_scaling) / 360.0) * (
                    self.solar_system_object_link.planets["earth"].planet_diameter_km / float(self.planet_diameter_km))
        #        print "ratio " + str(ratio)
        large_blast_surface = pygame.transform.scale(blast_surface, (
        int(blast_surface.get_width() * ratio / 2), int(blast_surface.get_height() * ratio / 2)))
        medium_blast_surface = pygame.transform.scale(blast_surface, (
        int(blast_surface.get_width() * ratio / 3), int(blast_surface.get_height() * ratio / 3)))
        small_blast_surface = pygame.transform.scale(blast_surface, (
        int(blast_surface.get_width() * ratio / 4), int(blast_surface.get_height() * ratio / 4)))
        if bases_involved is None:
            ...
            for base_instance in bases_involved.values():
                ...
                try:
                    ...
                    edges = [base_pos[0], base_pos[1], base_pos[0],
                             base_pos[1]]  # ie. left, bottom, right, top edge in spherical coordiantes
                else:
        ...
        if self.projection_scaling <= 360:  # for the round world projection
            ...
            proj_hit_locations = self.sphere_to_plane_total(hit_locations, self.eastern_inclination,
                                                            self.northern_inclination, self.projection_scaling)
            for i in range(len(proj_hit_locations)):
                proj_hit_locations[i] = (
                proj_hit_locations[i][0] + global_variables.window_size[0] / 2 - self.projection_scaling / 2,
                proj_hit_locations[i][1] + global_variables.window_size[1] / 2 - self.projection_scaling / 2)
        ...
        if per_hit_intensity > 50:
            ...
            for proj_hit_location in proj_hit_locations:
                proj_hit_location_transposed = (int(proj_hit_location[0] - small_blast_surface.get_width() / 2),
                                                int(proj_hit_location[1] - small_blast_surface.get_height() / 2))
                ...
                #                renderer.update()
                pygame.time.delay(33)
                proj_hit_location_transposed = (int(proj_hit_location[0] - medium_blast_surface.get_width() / 2),
                                                int(proj_hit_location[1] - medium_blast_surface.get_height() / 2))
                mainscreen.blit(medium_blast_surface, proj_hit_location_transposed)
                pygame.display.flip()
                #                mainscreen.set_picture(surface)
                #                renderer.update()
                pygame.time.delay(100)
                proj_hit_location_transposed = (int(proj_hit_location[0] - large_blast_surface.get_width() / 2),
                                                int(proj_hit_location[1] - large_blast_surface.get_height() / 2))
...
#
# for i, sphere_hit_location in enumerate(sphere_hit_locations): if isinstance(sphere_hit_location,
# tuple): size_of_blast = random.choice(["small","medium","large"]) if size_of_blast == "small":
# sphere_hit_location_transposed = (int(sphere_hit_location[0] - small_blast_surface.get_width()/2),
# int(sphere_hit_location[1] - small_blast_surface.get_height()/2)) surface.blit(small_blast_surface,
# sphere_hit_location_transposed) elif size_of_blast == "medium": sphere_hit_location_transposed = (int(
# sphere_hit_location[0] - medium_blast_surface.get_width()/2), int(sphere_hit_location[1] -
# medium_blast_surface.get_height()/2)) surface.blit(medium_blast_surface,sphere_hit_location_transposed) elif
# size_of_blast == "large": sphere_hit_location_transposed = (int(sphere_hit_location[0] -
# large_blast_surface.get_width()/2), int(sphere_hit_location[1] - large_blast_surface.get_height()/2)) surface.blit(
# large_blast_surface,sphere_hit_location_transposed) else: raise Exception("Weird")
#
#                mainscreen.set_picture(surface)
#                renderer.update()
