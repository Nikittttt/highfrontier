...
class IntroGui:
    ...
    def __init__(self):
        ...
        self.gui_rect = pygame.Rect(global_variables.window_size[0] / 2 - 90, global_variables.window_size[1] / 3, 180,
                                    180)
    ...
    def initialize_intro_sequence(self):
        ...
        if not all_ok:
            print(
                "WARNING: Some files were missing from the intro directory. It will be recreated. This will take time.")
            ...
            for i in range(self.steps_system):
                ...
                surface = self.sol.draw_solar_system(zoom_level=zoom, date_variable=self.sol.current_date,
                                                     center_object="earth")
                pickle_final_name_and_path = os.path.join("intro",
                                                          "intro_file_" + str(self.triplify_number(i)) + ".jpg")
            ...
            for i in range(self.steps_both):
                ...
                surface = self.sol.draw_solar_system(zoom_level=zoom, date_variable=self.sol.current_date,
                                                     center_object="earth")
                surface.blit(pygame.Surface((projection_scaling * 4, projection_scaling * 4)), (
                ...
                surface.blit(planet_surface, (global_variables.window_size[0] / 2 - projection_scaling / 2,
                                              global_variables.window_size[1] / 2 - projection_scaling / 2))
                pickle_final_name_and_path = os.path.join("intro", "intro_file_" + str(
                    self.triplify_number(i + self.steps_system)) + ".jpg")
                pygame.image.save(surface, pickle_final_name_and_path)
            for i in range(self.steps_planet):
                ...
                surface.blit(planet_surface, (global_variables.window_size[0] / 2 - projection_scaling / 2,
                                              global_variables.window_size[1] / 2 - projection_scaling / 2))
                pickle_final_name_and_path = os.path.join("intro", "intro_file_" + str(
                    self.triplify_number(i + self.steps_system + self.steps_both)) + ".jpg")
                pygame.image.save(surface, pickle_final_name_and_path)
    ...
    def background_sequence(self, i):
        ...
        bottom_rect = pygame.Rect(0, self.gui_rect[1] + self.gui_rect[3], global_variables.window_size[0],
    ...
    def load_callback(self, label, function_parameter):
        self.gui_rect = pygame.Rect(global_variables.window_size[0] / 2 - 150, global_variables.window_size[1] / 3 - 50,
                                    300, 300)
        pygame.draw.rect(self.window, (212, 212, 212), self.gui_rect)
        load_window = gui_components.fast_list(self.window, os.listdir("savegames"),
                                               rect=pygame.Rect(self.gui_rect[0], self.gui_rect[1], self.gui_rect[2],
                                                                self.gui_rect[3] - 50))
        self.buttons = {}
    ...
    def create_intro_gui(self, label=None, function_parameter=None):
        ...
        for i, button_name in enumerate(button_names):
            self.buttons[button_name] = gui_components.button(button_name,
                                                              ...
                                                              function_parameter=None,
                                                              topleft=(
                                                              self.gui_rect[0] + 25, self.gui_rect[1] + i * 35 + 25),
                                                              fixed_size=(130, 30))
    ...
    def ask_country_name(self, label, function_parameter):
        ...
        country_window = gui_components.fast_list(self.window, self.countries,
                                                  rect=pygame.Rect(self.gui_rect[0], self.gui_rect[1], self.gui_rect[2],
                                                                   self.gui_rect[3] - 50))
        self.buttons = {}
    ...
    def ask_company_capital(self, label, function_parameter, give_warning=False):
        ...
        if all_ok:
            ...
            if give_warning:
                warning_label = global_variables.standard_font_small.render("Starting capital must be an", True,
                                                                            (0, 0, 0))
                self.window.blit(warning_label, (self.gui_rect[0] + 10, self.gui_rect[1] + 90))
