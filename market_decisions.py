...
class market_decisions:
    ...
    def __init__(self):
        ...
        self.calculate_demand_reaction = {1: self.calculate_demand_reaction_01, 2: self.calculate_demand_reaction_02,
                                          3: self.calculate_demand_reaction_03, 4: self.calculate_demand_reaction_04}
        self.calculate_intercity_demand = {1: self.calculate_intercity_demand_01}
        self.calculate_supply_reaction = {1: self.calculate_supply_reaction_01, 2: self.calculate_supply_reaction_02,
                                          3: self.calculate_supply_reaction_03}
    ...
    def pick_research_01(market_decision_class, self):
        ...
        if len(self.known_technologies) == 0:
            ...
            if self.solar_system_object_link.message_printing["debugging"]:
                print_dict = {"text": "WARNING: the " + str(
                    self.name) + " does not know any technologies to begin with. This must be a mistake",
                              "type": "debugging"}
            ...
            while destination is None:
                chosen_origination_point_name = random.choice(list(self.known_technologies.items()))[0]
                chosen_origination_point = self.solar_system_object_link.technology_tree.vertex_dict[
                    chosen_origination_point_name]
                destination = self.solar_system_object_link.technology_tree.get_research_project(
                    chosen_origination_point_name, self.known_technologies)
                i = i + 1
                if i > 50:
                    if self.solar_system_object_link.message_printing["debugging"]:
                        print_dict = {
                            "text": "WARNING: It does not seem like there are any technologies left to research for " + str(
                                self.name) + " perhaps use the implode_and_expand function?", "type": "debugging"}
                        self.solar_system_object_link.messages.append(print_dict)

                    break
            if destination is not None:
                self.target_technology = destination["target_technology"]
                self.target_technology_cost = int(
                    destination["distance"] * self.solar_system_object_link.technology_research_cost)
    ...
    def start_research_firms_01(market_decision_class, self):
        ...
        if research_ratio < self.company_database["desired_research"]:
            ...
            home_city_names = list(self.home_cities.items())

            try:
                location_choice_name = random.choice(home_city_names)[0]
            except:
                print(self.name)
                print(self.home_cities)
                raise Exception(
                    "This bug has been observed before and probably results from a company not having any home_cities")
    ...
    def evaluate_commodities_market_01(market_decision_class, self):
        ...
        """
        quickness_to_start_firm = (101 - self.company_database[
            "quickness_to_start_firm"]) / 20.0  # ratio of buy_offers to sell_offers at which a new firm will be started
        size_of_started_firm = self.company_database[
                                   "size_of_started_firm"] / 100.0  # decision of the scaling factor of the size of a new-started firm. If one, it will match the output to the quantity currently sought on the market
        ...
        for base in self.home_cities.values():
            for resource in base.market["buy_offers"]:
                ...
                if len(useful_processes) > 0:
                    useful_processes_keys = sorted(useful_processes.items())
                    chosen_process_name = useful_processes[useful_processes_keys[-1][0]]
                    ...
                    decided_scaling_factor = int(
                        int(quantity_wanted_on_market / (process_here["output"][resource])) * size_of_started_firm * (
                                    random.random() * 0.90 + 0.1))
    ...
    def evaluate_tech_market_01(market_decision_class, self):
        ...
        """
        sell_tech = random.randint(1,
                                   100) < 4  # FIXME opportunity for tuningparameter ("how often to check for tech sales")
        buy_tech = random.randint(1,
                                  100) < 10  # FIXME opportunity for tuningparameter ("how often to check for tech buys")
        ...
        if sell_tech:
            ...
            for base_instance in self.home_cities.values():
                ...
                try:
                    base_instance.market["sell_offers"]["labor"]
                except:
                    pass
                else:
                ...
                if labor_price_here is None:
                    try:
                        base_instance.market["buy_offers"]["labor"]
                    except:
                        pass
                    else:
                ...
                for technology_name in self.known_technologies:
                    ...
                    if technology["known_by"] != "everybody":
                        ...
                        if known_by_number > 10:
                            known_by_number = 10
                        price = ((mean_labor_price / research_point_per_labor) * (
                                    self.solar_system_object_link.technology_research_cost / 1000)) * (
                                            (known_by_number - 9) ** 2)
                        print_dict = {
                            "text": self.name + " considers selling " + technology_name + " for " + str(price),
                            "type": "tech discovery"}
                        self.solar_system_object_link.messages.append(print_dict)
                        ...
                        if technology["technology_name"] not in self.known_technologies.keys():
                            ...
                            if winning_price < self.capital:
                                check_result = self.solar_system_object_link.technology_tree.check_technology_bid(
                                    self.known_technologies, technology)
                                if check_result == "ok":
                                    ...
                                    if self == self.solar_system_object_link.current_player:
                                        print_dict = {
                                            "text": str(technology["technology_name"]) + " is not known by " + str(
                                                self.name) + ". It was bought from " + selling_company.name + " at a price of " + str(
                                                winning_price), "type": "tech discovery"}
                                        self.solar_system_object_link.messages.append(print_dict)
                        ...
                        if buyer_base.name in self.home_cities.keys():  # ok this proves that it is within the network of the company
                            ...
                            for base_instance in [buyer_base, seller_base]:
                                if len(base_instance.market["sell_offers"][trade_route["transport_type"]]) > 0:
                                    transport_prices.append(
                                        base_instance.market["sell_offers"][trade_route["transport_type"]][0]["price"])
                            if len(transport_prices) > 0:
                                transport_unit_price = min(transport_prices)
                            else:
                                transport_unit_price = \
                                self.solar_system_object_link.trade_resources[trade_route["transport_type"]][
                                    "starting_price"]
                            ...
                            for resource in seller_base.market["sell_offers"].keys():
                                if self.solar_system_object_link.trade_resources[resource]["transportable"]:
                                    ...
                                    if connection not in already_connected:
                                        if len(seller_base.market["sell_offers"][resource]) > 0 and len(
                                                buyer_base.market["buy_offers"][resource]) > 0:
                                            ...
                                            if buyprice > 0:
                                                if (profit_margin / buyprice) > (
                                                        tendency_to_start_trade_routes / 100.0):  # only consider the trade if the profit_margin is a higher percentage of the buyprice than the tendency to start trades
                                                    ...
                                                    while name_is_not_unique:
                                                        ...
                                                        for company_instance in self.solar_system_object_link.companies.values():
                                                            if name in company_instance.owned_firms.keys():
                                                                name_is_not_unique = True
                                                    owner = self
                                                    input_output_dict = {"input": {}, "output": {}, "timeframe": 30,
                                                                         "byproducts": {}}
                                                    ...
                                                    new_merchant_firm = company.merchant(self.solar_system_object_link,
                                                                                         seller_base, buyer_base,
                                                                                         input_output_dict, owner, name,
                                                                                         transport_type, distance,
                                                                                         resource)
                                                    self.owned_firms[name] = new_merchant_firm
                                                    if self == self.solar_system_object_link.current_player:
                                                        print_dict = {
                                                            "text": self.name + " started up a merchant between " + str(
                                                                seller_base.name) + " and " + str(buyer_base.name),
                                                            "type": "tech discovery"}
                                                        self.solar_system_object_link.messages.append(print_dict)
    ...
    def evaluate_firms_01(market_decision_class, self):
        ...
        for firm_name in self.owned_firms:
            ...
            if not isinstance(firm_instance,
                              company.research):  # do not close research firms here, since they will always give negative results (this is done in the start_research_firms function
        ...
        for firm_to_close in close_these_firms:
            if not isinstance(self.owned_firms[firm_to_close], company.base):
                ...
                self.owned_firms[firm_to_close].for_sale_deadline = datetime.timedelta(
    ...
    def calculate_demand_reaction_01(market_decision_class, self):
        ...
        current_date = self.solar_system_object_link.current_date
        timeframe_considered = int(
            self.input_output_dict["timeframe"] * (self.owner.company_database["timeframe_considered"] / 20.0))
        ...
        for resource in self.input_output_dict["input"]:
            if self.input_output_dict["input"][resource] * timeframe_considered > self.stock_dict[resource]:
                if not isinstance(self, company.base_construction):
                    quantity_wanted = int(
                        self.input_output_dict["input"][resource] * timeframe_considered - self.stock_dict[
                            resource]) + 1
                else:
                    quantity_wanted = self.input_output_dict["input"][resource] - self.stock_dict[resource]
                try:
                    market["sell_offers"][resource][0]
                except:
                ...
                if quantity_wanted * price <= self.owner.capital:
                    buy_offer = {"resource": resource, "price": float(price), "buyer": self, "name": self.name,
                                 "quantity": int(quantity_wanted), "date": current_date}
        ...
        for resource in self.input_output_dict["input"]:
            ...
            if self.stock_dict[resource] < self.input_output_dict["input"][resource] * timeframe_considered:
                ...
                if quantity_wanted > 1:
                    if quantity_wanted * price <= self.owner.capital:
                        if not (isinstance(quantity_wanted, int)):
                            if self.solar_system_object_link.message_printing["debugging"]:
                                print_dict = {
                                    "text": "DEBUGGING WARNING: in calculate_demand_reaction_02 quantity_wanted: " + str(
                                        quantity_wanted), "type": "debugging"}
                                self.solar_system_object_link.messages.append(print_dict)

                        if quantity_wanted < 0:
                            if self.solar_system_object_link.message_printing["debugging"]:
                                print_dict = {
                                    "text": "DEBUGGING WARNING: in calculate_demand_reaction_02 quantity_is negative. VERY WEIRD",
                                    "type": "debugging"}
                                self.solar_system_object_link.messages.append(print_dict)
            ...
            if quantity_wanted > 1:
                if quantity_wanted * price <= self.owner.capital:
                    if quantity_wanted < 0:
                        if self.solar_system_object_link.message_printing["debugging"]:
                            print_dict = {
                                "text": "DEBUGGING WARNING: in calculate_demand_reaction_03 quantity_is negative. VERY WEIRD",
                                "type": "debugging"}
                            self.solar_system_object_link.messages.append(print_dict)
    ...
    def calculate_demand_reaction_04(market_decision_class, self):
        ...
        for resource in self.input_output_dict["output"]:
            ...
            try:
                market["buy_offers"][resource][0]
            except:
                pass
            else:
                market_price_of_output = market["buy_offers"][resource][0]["price"] * self.input_output_dict["output"][
                    resource] + market_price_of_output

        chosen_price_of_input_total = market_price_of_output * (
                    random.random() * 0.2 + 0.7)  # FIXME this can be made genetical
        ...
        for resource in self.input_output_dict["input"]:
            if self.input_output_dict["input"][resource] * timeframe_considered > self.stock_dict[resource]:
                ...
                # print self.name + " used calculate_demand_reaction_04 to set a price of " + resource + " to " +
                # str(int(price)) + " because the output could be sold for " + str(int(market_price_of_output)) + " -
                # the input_volume is " + str(input_volume) + " and the output_volume is " + str(output_volume) + "
                # so each output could be sold for " + str(int(market_price_of_output/output_volume))
                else:
                    price = float(market["sell_offers"][resource][0]["price"])
        ...
        for resource in resources:
            ...
            if resource == self.resource:
                ...
                if self.owner == self.solar_system_object_link.current_player:
                    print_dict = {"text": "the transport quantity wanted was " + str(
                        quantity_wanted) + " and self.from_stock_dict[self.resource] was " + str(
                        self.from_stock_dict[self.resource]), "type": "firm info"}
                    self.solar_system_object_link.messages.append(print_dict)
            ...
            if quantity_wanted > 1:
                if quantity_wanted * price <= self.owner.capital:
                    if quantity_wanted < 0:
                        if self.solar_system_object_link.message_printing["debugging"]:
                            print_dict = {
                                "text": "DEBUGGING WARNING: in calculate_demand_reaction_03 quantity_is negative. "
                                        "VERY WEIRD",
                                "type": "debugging"}
                            self.solar_system_object_link.messages.append(print_dict)
                    ...
                    if self.owner == self.solar_system_object_link.current_player:
                        print_dict = {
                            "text": self.name + " is buying " + resource + " at " + self.from_location.name + " at price " + str(
                                int(price)) + " and qt " + str(quantity_wanted), "type": "firm info"}
                        self.solar_system_object_link.messages.append(print_dict)
            ...
            if production_level > 0:
                ...
                if surplus_ratio > 0:
                    ...
                    if quantity_offered > 1 and price > 0:
                        sell_offer = {"resource": resource, "price": float(price), "seller": self, "name": self.name,
                                      "quantity": int(quantity_offered), "date": current_date}
    ...
    def calculate_intercity_supply_01(market_decision_class, self):
        ...
        if quantity_offered > 1 and price > 0:
            ...
            if self.owner == self.solar_system_object_link.current_player:
                print_dict = {
                    "text": self.name + " is selling " + resource + " at " + self.to_location.name + " at price " + str(
                        int(price)) + " and qt " + str(quantity_offered), "type": "firm info"}
                self.solar_system_object_link.messages.append(print_dict)
