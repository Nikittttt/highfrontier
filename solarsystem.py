...
class solarsystem:
    ...
    def evaluate_each_game_step(self):
        ...
        try:
            self.last_evaluation_day_for_evaluate_each_game_step
        except:
            ...
            if self.current_date.year != self.last_evaluation_day_for_evaluate_each_game_step.year:
                self.last_evaluation_day_for_evaluate_each_game_step = self.current_date
                print_dict = {
                    "text": "running the " + str(self.current_date.year) + " yearly part of evaluate_each_game_step",
                ...
                number_of_companies = number_of_companies - len(
                    self.companies)  # do not count the country/companies already in existence
                ...
                if number_of_companies > 0:
                    ...
                    richness_list = sorted(richness_database.keys())
                    richness_list = richness_list[int(len(richness_list) * 2 / 3):len(
                        richness_list)]  # "This is the richest 1/3 of all companies"
                    ...
                    for i in range(0, number_of_companies):
                        ...
                        new_company = company.company(self, model_company_database=model_company.company_database,
                                                      capital=10000000)  # FIXME change capital
                        self.companies[new_company.name] = new_company
                        list_of_model_companies_names.append(model_company.name)
                    print_dict = {"text": "Made " + str(
                        len(list_of_model_companies_names)) + " new companies on the model of: " + str(
                        list_of_model_companies_names), "type": "general company info"}
                    self.messages.append(print_dict)
    ...
    def __init__(self, start_date, de_novo_initialization=True):
        if de_novo_initialization:
            ...
            self.bitterness_of_world = (
                None, None)  # how much bitterness there is in the world. First entry is lowest bitterness level
            ...
            if os.access(os.path.join("data", "economy", "trade resources.txt"), os.R_OK):
                ...
                raise Exception(
                    "Did not find trade_resources.txt file. Warning - this might endanger the integrity of the program")
    ...
    def initialize_planets(self):
        ...
        for planet_name in read_planet_database:
            planet_instance = planet.planet(planet_name, self,
                                            read_planet_database[planet_name])  # creating the planet instance
            try:
                global_variables.distance_data
            except:
                global_variables.distance_data = planet_instance.calculate_all_distances()
            planet_database[planet_name] = planet_instance
        # checking that all projections exist
        random_planet_name = random.choice(list(planet_database.items()))[0]
    ...
    def initialize_companies(self):
        ...
        random_companyName = random.choice(list(read_company_database.items()))[0]
        for key in read_company_database[random_companyName]:
            if len(key) > global_variables.max_letters_in_company_names:
                raise Exception(key + " is " + str(len(key)) + " - maximum is " + str(
                    global_variables.max_letters_in_company_names))
        # print read_company_database
        company_database = {}
        for companyName in read_company_database:
            company_instance = company.company(self, companyName=companyName,
                                               model_company_database=read_company_database[companyName],
                                               capital=country_to_GNP_list[
                                                   companyName])  # creating the company instance
            for planet in self.planets.values():
                for base in planet.bases.values():
        ...
        for companyName in country_to_base_list:
            if companyName not in company_database:
                single_company_data = {}
                company_instance = company.company(self, model_company_database=model_company_data,
                                                   companyName=companyName, capital=country_to_GNP_list[
                        companyName])  # creating the company instance
        ...
        if number_of_companies > global_variables.max_number_of_companies:
            number_of_companies = global_variables.max_number_of_companies
        number_of_companies = number_of_companies - len(
            company_database)  # do not count the country/companies already included
        GWP = 0
        ...
        for i in range(0, number_of_companies):
            ...
            if capital < 1000:
                capital = 1000
            home_city = random.choice(list(all_bases.items()))[0]
            ...
            for technology in company_instance.known_technologies:
                if technology != "common knowledge":
                    ...
                    if True:
                        ...
                        location_name = list(company_instance.home_cities.items())[0][0]
    ...
    def save_solar_system(self, filename, prepare_scenario=False):
                    ...
                    if sub_entry_name[0:2] != "__":
                        ...
                        try:
                            ...
                            if issubclass(sub_entry.__class__, Image.Image):
                                print("The entry_name " + str(sub_entry_name) + " is a " + str(
                                    sub_entry.__class__) + " it is found at " + str(before))
        ...
        if prepare_scenario:
            ...
            self.current_date = datetime.date(time.localtime()[0], time.localtime()[1],
                                              time.localtime()[2])  # this is just for use when creating start out games
        ...
        known_planet_images = ["wet_areas",
                               "topo_image"]  # this can be tested with above commented out block - also remember to change accordingly in the load function
        for planet_instance in self.planets.values():
            ...
            for known_planet_image in known_planet_images:
                try:
                    getattr(planet_instance, known_planet_image)
                except:
                    pass
                else:
        ...
        try:
            ...
            print(
                "DEBUGGING: cPickle failed, likely because of memory issue. Switching to regular pickle. Regular pickle is slower but less memory intensive.")
            # import pickle
            try:
                pickle.dump(self, file)
            except:
                print("DEBUGGING: regular pickle also failed. The game was NOT saved")
            else:
                print("game saved")
        except Exception:
    ...
    def load_solar_system(self, filename):
        ...
        try:
            ...
            print_dict = {"text": "Un-loadable file: " + str(filename) + " - no load performed",
                          "type": "general gameplay info"}
    ...
    def get_satellite_to_center_position(self, object, zoom_level, date_variable):
        ...
        relative_semi_major_axis = float(semi_major_axis * max(self.window_size) / (
                17000000000 / zoom_level))  # this setting is made so zoom_level = 1 gives a view of all bodies including pluto
    ...
    def get_areas_of_interest(self, zoom_level, date_variable, center_object):
        ...
        for orbit_center in hierarchy_list:
            list = self.calculate_from_a_center(zoom_level, date_variable, surface=None, orbit_center=orbit_center,
                                                hierarchy_dependent_transposition=hierarchy_dependent_transposition,
                                                draw=False)
            positions_of_interest.update(list)
            if orbit_center != "sun":
                new_hierarchy_dependent_transposition = self.get_satellite_to_center_position(orbit_center, zoom_level,
                                                                                              date_variable)
                hierarchy_dependent_transposition = (
                    new_hierarchy_dependent_transposition[0] + hierarchy_dependent_transposition[0],
                    new_hierarchy_dependent_transposition[1] + hierarchy_dependent_transposition[1])
        # Include the sun
        sun_position = (hierarchy_dependent_transposition[0] + global_variables.window_size[0] * 0.5,
                        hierarchy_dependent_transposition[1] + global_variables.window_size[1] * 0.5)
        positions_of_interest["sun"] = sun_position
        # assign the rectangles
        for planet in positions_of_interest:
            areas_of_interest_here[(positions_of_interest[planet][0], positions_of_interest[planet][1], size_of_target,
                                    size_of_target)] = planet
        self.areas_of_interest = areas_of_interest_here

    def calculate_from_a_center(self, zoom_level, date_variable, surface, orbit_center,
                                hierarchy_dependent_transposition, draw=True):
        """
        ...
        for object in self.planets:
            if self.planets[object].planet_data[
                "satellite_of"] == orbit_center:  # draw all the satellites of each of the preceding planets
                semi_major_axis = self.planets[object].planet_data["semi_major_axis"]  # in km
                relative_semi_major_axis = float(semi_major_axis) * max(self.window_size) / (
                        17000000000 / zoom_level)  # this setting is made so zoom_level = 1 gives a view of all bodies including pluto
                eccentricity = self.planets[object].planet_data["eccentricity"]
                ...
                satellite_diameter = self.planets[object].planet_diameter_km * max(self.window_size) * int(
                    zoom_level) / 17000000000
                orbital_position = self.get_planet_position(object, date_variable)
                orbit_placement = int((orbital_position * 99) / (2 * math.pi))
                orbit = []
                for i in range(0, 100):
                    t = -math.pi + i * ((2 * math.pi) / 100)
                    x = self.window_size[0] / 2 + relative_semi_major_axis * math.cos(t) + \
                        hierarchy_dependent_transposition[0]
                    y = self.window_size[1] / 2 + relative_semi_minor_axis * math.sin(t) + \
                        hierarchy_dependent_transposition[1]
                    orbit.append((x, y))
                if draw:
                    for i in range(0, len(orbit)):
                        if i == orbit_placement:
                            x = self.window_size[0] / 2 + relative_semi_major_axis * math.cos(
                                orbital_position - math.pi) + hierarchy_dependent_transposition[0]
                            y = self.window_size[1] / 2 + relative_semi_minor_axis * math.sin(
                                orbital_position - math.pi) + hierarchy_dependent_transposition[1]
                ...
                if not draw:
                    x = self.window_size[0] / 2 + relative_semi_major_axis * math.cos(orbital_position - math.pi) + \
                        hierarchy_dependent_transposition[0]
                    y = self.window_size[1] / 2 + relative_semi_minor_axis * math.sin(orbital_position - math.pi) + \
                        hierarchy_dependent_transposition[1]
                    if 0 < x < self.window_size[0] and 0 < y < self.window_size[1]:
    ...
    def draw_small_object(self, diameter, position, object, surface):
        ...
        if self.solar_system_zoom > 2000000:
            ...
            pygame.draw.polygon(surface, smallplanet_color,
                                [position, (position[0], position[1] + 1), (position[0] + 1, position[1]),
            ...
            if object == self.current_planet.planet_name:
                ...
                custom_drawn_planet = self.planets[object].draw_image(0, 90, projection_scaling_here,
                                                                      fast_rendering=True)
                surface.blit(custom_drawn_planet,
                             (position[0] - projection_scaling_here / 2, position[1] - projection_scaling_here / 2))
        else:
            pointlist = []
            for i in range(1, 50):
    ...
    def draw_solar_system(self, zoom_level, date_variable, center_object):
        ...
        hierarchy_dependent_transposition = (0, 0)
        for orbit_center in hierarchy_list:
            surface = self.calculate_from_a_center(zoom_level, date_variable, surface, orbit_center,
                                                   hierarchy_dependent_transposition, True)
            if orbit_center != "sun":
                new_hierarchy_dependent_transposition = self.get_satellite_to_center_position(orbit_center, zoom_level,
                                                                                              date_variable)
                hierarchy_dependent_transposition = (
