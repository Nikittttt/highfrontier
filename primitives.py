...
def make_linear_x_axis(surface, frame_size, xlim, solar_system_object_link, unit=""):
    ...
    for i in range(0, tick_marks):
        ...
        pygame.draw.line(surface, (0, 0, 0), (tick_mark_placement, size[1] - frame_size - tick_mark_width),
                         (tick_mark_placement, size[1] - frame_size + tick_mark_width))
        x_correction = 0
        y_correction = 0
        surface.blit(tick_mark_rendered_value, (tick_mark_placement - tick_mark_rendered_value_size[0] / 2,
                                                size[1] - frame_size + tick_mark_rendered_value_size[1]))
...
def make_linear_y_axis(surface, frame_size, ylim, solar_system_object_link, unit=""):
    ...
    for i in range(0, tick_marks):
        ...
        pygame.draw.line(surface, (0, 0, 0), (frame_size - tick_mark_width, tick_mark_placement),
                         (frame_size + tick_mark_width, tick_mark_placement))

        if frame_size - tick_mark_width * 2 - tick_mark_rendered_value_size[
            0] < 0:  # to make sure that the unit doesn't wrap off screen
            x_correction = -(frame_size - tick_mark_width * 2 - tick_mark_rendered_value_size[0])
            y_correction = (size[1] / 100)
        else:
            x_correction = 0
            y_correction = 0
        surface.blit(tick_mark_rendered_value, (
        frame_size - tick_mark_width * 2 - tick_mark_rendered_value_size[0] + x_correction,
        tick_mark_placement - tick_mark_rendered_value_size[1] / 2 - y_correction))

    if unit not in ["", "date"]:
        unit_rendered_value = global_variables.standard_font.render(unit, True, (0, 0, 0))
        unit_rendered_value_size = global_variables.standard_font.size(unit)
        surface.blit(unit_rendered_value, (
        frame_size - unit_rendered_value_size[0], frame_size - unit_rendered_value_size[1] - (size[1] / 50)))
...
def listify_tabular_data(data, size, manager, sort_by="rownames", column_order=None, interval=(0, 200),
                         reverse_sort=False):
    """
